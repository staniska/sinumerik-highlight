...
import {drawChanges, runBoundingContourParse} from "./interpretator";
...
import {normalizeFileName, readProgramLines, extractBoundingContourBlock} from "./utils";

const path = require('path')

// Bounding-contour preflight: count WHILE/FOR nesting depth at callRow.
// REPEAT is not handled statically (its block is between a label above
// and the REPEAT line below); the runtime visited-counter guard in
// parseRows catches that case via boundingContourRequest.error.
function _isInsideLoop(lines, callRow) {
    let depth = 0
    for (let i = 0; i < callRow && i < lines.length; i++) {
        const line = lines[i].split(';')[0].toUpperCase()
        const ends = line.match(/\bEND(WHILE|FOR)\b/g)
        const starts = line.match(/\b(WHILE|FOR)\b/g)
        if (ends) depth -= ends.length
        if (starts) depth += starts.length
    }
    return depth > 0
}
...
export default class SinumerikView {
    ...
    constructor(serializedState) {
        ...
        if (Editor) {
            ...
            atom.workspace.onDidChangeActiveTextEditor(function (editor) {
                ...
                View.sinumerikView.suggestUnpinIfUnrelated(Editor)
    ...
    async createBoundingContour(hit) {
        const editor = atom.workspace.getActiveTextEditor()
        if (!editor) {
            atom.notifications.addWarning('No active editor.')
            return
        }
        const callerFile = editor.getPath()
        if (!callerFile) {
            atom.notifications.addWarning('Active editor has no file path.')
            return
        }
        if (View.sinumerikView.parseData?.filename !== callerFile) {
            atom.notifications.addWarning(
                'Bounding contour can only be created from the active main program.'
            )
            return
        }

        // Validate the subroutine has a bounding-contour block before we
        // trigger an expensive reparse.
        let subLines
        try {
            subLines = await readProgramLines(hit.sub.path)
        } catch (err) {
            atom.notifications.addError(`Failed to read ${hit.sub.path}: ${err.message}`)
            return
        }
        if (!extractBoundingContourBlock(subLines)) {
            atom.notifications.addWarning(
                `No ;BOUNDING_CONTOUR_BEGIN/END block in ${hit.sub.name}.`
            )
            return
        }

        if (_isInsideLoop(editor.getText().split('\n'), hit.row)) {
            atom.notifications.addWarning(
                `Call at row ${hit.row + 1} is inside a loop — bounding contour not supported.`
            )
            return
        }

        View.sinumerikView.boundingContourRequest = {
            callerProgName: callerFile,
            callerFile,
            callRow:        hit.row,
        ...
        try {
            ...
            atom.notifications.addError(`Parse failed: ${err.message}`)
            View.sinumerikView.boundingContourRequest = null
            return
        }

        const req = View.sinumerikView.boundingContourRequest
        if (!req) return
        if (req.error) {
            atom.notifications.addWarning(req.error)
            View.sinumerikView.boundingContourRequest = null
            return
        }
        if (!req.snapshot) {
            atom.notifications.addWarning(
                `Call at row ${hit.row + 1} was not reached during parse (skipped by control flow?).`
            )
            View.sinumerikView.boundingContourRequest = null
            return
        }
        ...
        let bucket
        try {
            bucket = await runBoundingContourParse(req)
        } catch (err) {
            atom.notifications.addError(`Bounding mini-parse failed: ${err.message}`)
            View.sinumerikView.boundingContourRequest = null
            return
        }
        const reqError = View.sinumerikView.boundingContourRequest?.error || req.error
        View.sinumerikView.boundingContourRequest = null

        if (reqError) {
            atom.notifications.addWarning(reqError)
            return
        }
        if (!bucket) {
            atom.notifications.addWarning(`Failed to generate bounding contour for ${hit.sub.name}.`)
            return
        }

        try {
            await updateContourEditData()
        } catch (err) {
            console.warn('Bounding contour: updateContourEditData failed', err)
        }

        atom.notifications.addSuccess(
            `Bounding contour for ${hit.sub.name} (row ${hit.row + 1}) generated.`
        )
    }
    ...
    suggestUnpinIfUnrelated(editor) {
        const pinned = this.sldPinnedFile
        if (!pinned) return

        const newPath = editor.getPath?.()
        if (!newPath || newPath === pinned) return

        const calledSubs = View.sinumerikView.parseData?.calledSubroutines
        const isSubroutine = calledSubs?.has(newPath)
        if (isSubroutine) return

        const pinnedName = path.basename(pinned)
        const newName    = path.basename(newPath)

        const notification = atom.notifications.addInfo(
            `SLD pinned to ${pinnedName}`,
            {
                description: `"${newName}" is not a subroutine of the pinned file. Unpin to follow this file?`,
                dismissable: true,
                buttons: [
                    {
                        text: 'Unpin',
    ...
    toggleSldPin() {
        ...
        if (this.sldPinnedFile) {
            // Unpin — clear pin first, then fire changeEditor so SLD
            // immediately switches to the currently active editor.
            ...
            if (this.activeTab === 'singleLine') {
                this.eventRouter.route('{"emitter": "singleLine", "event": "changeEditor"}')
            }
        } else {
            // Pin current file
            const filename = Editor.getPath()
