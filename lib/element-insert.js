...
export function insertRnd(element_1, element_2, programName) {
    ...
    if (element_1.type.match(/G[23]/) && element_2.type == 'G1') {
        ...
        if ((distance_From_CircleCenter_To_RND_Line >= 0 && element_1.type == 'G3') ||
            ...
            RND_start_ang = ang_norm;
        } else {
            RND_start_ang = ang_norm + 180;
        }
        if (RND_start_ang > 360) {
            RND_start_ang -= 360;
        }
        ...
        var RND_end_ang;
        if (RND_center_radius_factor < 0) {
            RND_end_ang = ang_To_RND_center;
        } else {
            RND_end_ang = ang_To_RND_center + 180;
        }
        console.log("Угол конца RND: " + RND_end_ang);

        //Угол раскрытия RND
        var RND_ang = Math.abs(RND_end_ang - RND_start_ang);
        ...
        var RND_ang_factor = Math.round((RND_end_ang - RND_start_ang) / RND_ang);
        ...
        //Тут правим входящую линию

        var LineStartInFrame = {};
        LineStartInFrame[axes[0]] = RND_center[axes[0]] + myMath.cos(RND_start_ang) * element_1.RND;
        LineStartInFrame[axes[1]] = RND_center[axes[1]] + myMath.sin(RND_start_ang) * element_1.RND;
        ...
        startInBase = getCoordinatesInBase({X:element_1.X_start, Y:element_1.Y_start,Z:element_1.Z_start});
