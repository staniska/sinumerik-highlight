...
export function insertRnd(element_1, element_2, programName) {
    ...
    if (element_1.type.match(/G[23]/) && element_2.type == 'G1') {
        ...
        //Центр окружности
        var jopa = 5;
        var canvasCircleCenter = {};
        canvasCircleCenter[axes[0]] = element_1[circleCenterAxes[0]];
        canvasCircleCenter[axes[1]] = element_1[circleCenterAxes[1]];
        canvasCircleCenter[axes[2]] = element_1[axes[2]];

        var circleCenterInBase = getCoordinatesInBase(canvasCircleCenter);
        canvasCircleCenter.X = circleCenterInBase[0];
        canvasCircleCenter.Y = circleCenterInBase[1];
        canvasCircleCenter.Z = circleCenterInBase[2];
        canvasCircleCenter.X_start = circleCenterInBase[0];
        canvasCircleCenter.Y_start = circleCenterInBase[1];
        canvasCircleCenter.Z_start = circleCenterInBase[2];
        canvasCircleCenter[axes[0]] += jopa;
        canvasCircleCenter[`${axes[0]}_start`] -= jopa;
        canvasCircleCenter.type = 'G1';
        canvasCircleCenter.row = element_1.row;
        // console.log(canvasCircleCenter);
        View.sinumerikView.parseData.canvas.push(JSON.parse(JSON.stringify(canvasCircleCenter)));
        canvasCircleCenter[axes[0]] -= jopa;
        canvasCircleCenter[`${axes[0]}_start`] += jopa;
        canvasCircleCenter[axes[1]] += jopa;
        canvasCircleCenter[`${axes[1]}_start`] -= jopa;
        // console.log(canvasCircleCenter);
        View.sinumerikView.parseData.canvas.push(JSON.parse(JSON.stringify(canvasCircleCenter)));
