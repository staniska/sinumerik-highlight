...
export function insertRnd(element_1, element_2, programName) {
    ...
    if (element_1.type.match(/G[23]/) && element_2.type == 'G1') {
        ...
        var ang_circleCenter_RND_center;
        var RND_center = {};

        console.log('Угол развала: ' + (line_ang - circleCenter_ang));
        // if (line_ang-circleCenter_ang < 90 && line_ang-circleCenter_ang > -90) {
        //     var ang_factor = 1;
        //     if (line_ang-circleCenter_ang < 0) {
        //         ang_factor = -1;
        //     }
        //     ang_circleCenter_RND_center = line_ang + 90 * ang_factor + ang_factor * myMath.acos((distanceFromCircleCenterToLine - element_1.RND) / (circleRadius - element_1.RND));
        //     // if ()
        //     console.log(ang_circleCenter_RND_center);
        //     RND_center[axes[0]] = element_1[circleCenterAxes[0]] + myMath.cos(ang_circleCenter_RND_center) * (circleRadius - element_1.RND);
        //     RND_center[axes[1]] = element_1[circleCenterAxes[1]] + myMath.sin(ang_circleCenter_RND_center) * (circleRadius - element_1.RND);
        // } else {
            var ang_factor = 1;
            if ((line_ang-circleCenter_ang > -180 && line_ang-circleCenter_ang < 0)) {
                ang_factor = -1;
            }
            console.log(ang_factor);
            var katet_factor = 1;
            ...
            // if (ang_circleCenter_lineStart_lineEnd < 0) {
            //     ang_circleCenter_lineStart_lineEnd += 360;
            // }
            // if (ang_circleCenter_lineStart_lineEnd > 180) {
            //     ang_circleCenter_lineStart_lineEnd -= 180;
            // }
            var katet_factor_2 = 1;
            if (ang_circleCenter_lineStart_lineEnd > 180) {
                ang_circleCenter_lineStart_lineEnd = 360 - ang_circleCenter_lineStart_lineEnd;
                katet_factor = -1;
                katet_factor_2 = -1;
            }


            console.log('УГОЛ!!!  ' + ang_circleCenter_lineStart_lineEnd);
            if (ang_circleCenter_lineStart_lineEnd < 90) {
                katet_factor = -1;
            }
            ...
            var first_katet = katet_factor * element_1.RND + circleRadius * myMath.sin(ang_circleCenter_lineStart_lineEnd);
            console.log('First katet: ' + first_katet);
            console.log(katet_factor);

            var point = {};
            point[axes[0]] = myMath.cos(ang_norm) * first_katet + element_1[circleCenterAxes[0]];
            point[axes[1]] = myMath.sin(ang_norm) * first_katet + element_1[circleCenterAxes[1]];
            console.log(point);
            var second_katet = Math.sqrt((circleRadius + katet_factor_2 * katet_factor * element_1.RND)**2 - first_katet ** 2);
            RND_center[axes[0]] = katet_factor_2 * katet_factor * myMath.cos(line_ang) * second_katet + point[axes[0]];
            RND_center[axes[1]] = katet_factor_2 * katet_factor * myMath.sin(line_ang) * second_katet + point[axes[1]];
        // }
