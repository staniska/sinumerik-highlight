...
export function checkMachine() {
    ...
    const filename = normalizeFileName(Editor.getTitle());
...
export function loadMachineData(callback) {
    const machineFilePath = atom.packages.getPackageDirPaths()[0] + '/sinumerik-highlight/userData/machines.json';
...
function machineOptionsHandler(event) {
    ...
    if (event === 'setActiveMachineButtonClick') {
        ...
        if (confirmDialog('Activate machine for this file? (file\'ll not changed)')) {
            filename = normalizeFileName(Editor.getTitle());
            View.sinumerikView.programmData[filename].machine = View.sinumerikView.machineData.machines[machineName];
...
async function createMachineDataFile() {
    const machineFilePath = atom.packages.getPackageDirPaths()[0] + '/sinumerik-highlight/userData/machines.json';
    const dirPath = atom.packages.getPackageDirPaths()[0] + '/sinumerik-highlight/userData';
    try {
        await fsPromises.mkdir(dirPath, {recursive: true});
        View.sinumerikView.machineData = {machines: {}};
        await fsPromises.writeFile(machineFilePath, JSON.stringify(View.sinumerikView.machineData));
