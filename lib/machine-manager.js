...
const machineDataDir  = () => atom.getConfigDirPath() + '/sinumerik-highlight';
const machineDataPath = () => machineDataDir() + '/machines.json';
const machineDataOldPath = () => atom.packages.getPackageDirPaths()[0] + '/sinumerik-highlight/userData/machines.json';
...
import {addOptions} from "./fanuc-options";
...
export function machineManager(event) {
    ...
    if (event === 'machineManagerMachineSearchButtonClick') {
        const text = View.sinumerikView.machineManagerMachineSearchInput.value;
...
function loadMachineDataFromProgrammFile() {
    if (confirmDialog('This function will add the machine in your macines list. Do you agree?')) {
        const Editor = atom.workspace.getActiveTextEditor();
        const filename = Editor.getPath();
...
function searchMachine(text, full) {
    ...
    for (let machine in View.sinumerikView.machineData.machines) {
        const machineName = View.sinumerikView.machineData.machines[machine].machineName;
...
export function loadMachineData(callback) {
    fs.readFile(machineDataPath(), function (err, data) {
        if (err) {
            if (err.code == 'ENOENT') {
                fsPromises.readFile(machineDataOldPath())
                    .then(async oldData => {
                        await fsPromises.mkdir(machineDataDir(), {recursive: true});
                        await fsPromises.writeFile(machineDataPath(), oldData);
                        console.log('sinumerik-highlight: machine data migrated to', machineDataPath());
                        loadMachineData(callback);
                    })
                    .catch(() => {
        ...
        try {
            ...
            if (!View.sinumerikView.machineManagerRightContainer.machineManagerMachinesDiv) {
                ...
                View.sinumerikView.machineManagerRightContainer.machineManagerMachinesDiv.machinesList.addEventListener('click', function (event) {
                    ...
                    if (View.sinumerikView.machineManagerRightContainer.machineManagerMachinesDiv.machinesList.getElementsByClassName('sinumerikLiSelected')[0]) {
                        View.sinumerikView.machineManagerRightContainer.machineManagerMachinesDiv.machinesList.getElementsByClassName('sinumerikLiSelected')[0].classList.remove('sinumerikLiSelected')
                    }
                    event.target.classList.add('sinumerikLiSelected')
                    machineOptions(event.target.innerText);
                    if (View.sinumerikView.machineData.defaultMachine && View.sinumerikView.machineData.defaultMachine === event.target.id) {
                        View.sinumerikView.machineManagerMainWindow.machineOptions.defaultMachineButton.classList.add('defaultMachineSelected')
                    } else {
                        View.sinumerikView.machineManagerMainWindow.machineOptions.defaultMachineButton.classList.remove('defaultMachineSelected')
                    }
                });
            ...
            while (View.sinumerikView.machineManagerRightContainer.machineManagerMachinesDiv.machinesList.children.length > 0) {
                const lastChild = View.sinumerikView.machineManagerRightContainer.machineManagerMachinesDiv.machinesList.lastChild;
                View.sinumerikView.machineManagerRightContainer.machineManagerMachinesDiv.machinesList.removeChild(lastChild);
                delete View.sinumerikView.machineManagerRightContainer.machineManagerMachinesDiv.machinesList[`${lastChild.id}`]
            }

            for (let machine in View.sinumerikView.machineData.machines) {
                const newMachineName = View.sinumerikView.machineData.machines[machine].machineName;
                ...
                View.sinumerikView.machineManagerRightContainer.machineManagerMachinesDiv.machinesList[`${newMachineName}`].id = newMachineName
                ...
                if (View.sinumerikView.machineData.defaultMachine &&
                    View.sinumerikView.machineData.defaultMachine === newMachineName) {
                    View.sinumerikView.machineManagerRightContainer.machineManagerMachinesDiv.machinesList[`${newMachineName}`].classList.add('icon-check')
                }
...
function machineOptionsHandler(event) {
    let sinumerikEventHandler = View.sinumerikView.eventRouter.route.bind(this);
    const machineName = View.sinumerikView.machineManagerMainWindow.machineOptions.header.textContent;
    ...
    if (event == 'attachMachineButtonClick') {
        ...
        if (confirmDialog('This function will add a comment to the end of your file. Do you agree?')) {
            const filename = Editor.getPath();
            if (View.sinumerikView.programmData[filename] === undefined) {
    ...
    if (event === 'setActiveMachineButtonClick') {
        ...
        if (confirmDialog('Activate machine for this file? (file\'ll not changed)')) {
...
function machineOptions(machineName) {
    ...
    if (!View.sinumerikView.machineManagerMainWindow.machineOptions) {
        ...
        View.sinumerikView.machineManagerMainWindow.machineOptions.defaultMachineButton = document.createElement('button');
        View.sinumerikView.machineManagerMainWindow.machineOptions.defaultMachineButton.className = 'sinumerikButton icon-check';
        View.sinumerikView.machineManagerMainWindow.machineOptions.defaultMachineButton.innerText = 'Select Default';
        View.sinumerikView.machineManagerMainWindow.machineOptions.defaultMachineButton.addEventListener('click', function () {
            // sinumerikEventHandler('{"emitter": "machineManager", "event": "defaultMachineButtonClick"}');
            const selectedMachine = View.sinumerikView.machineManagerRightContainer.machineManagerMachinesDiv.machinesList.querySelector('.sinumerikLiSelected').id
            if (View.sinumerikView.machineData && View.sinumerikView.machineData.defaultMachine === selectedMachine) {
                View.sinumerikView.machineData.defaultMachine = null
                View.sinumerikView.machineManagerMainWindow.machineOptions.defaultMachineButton.classList.remove('defaultMachineButton')
            } else {
                View.sinumerikView.machineData.defaultMachine = selectedMachine
            }
            saveMachineData(() => {
                View.sinumerikView.machineManagerRightContainer.machineManagerMachinesDiv.machinesList.querySelector(`#${selectedMachine}`).classList.add('sinumerikLiSelected')
            });
            //TODO доделать выбор дефолтной машины
        });
        View.sinumerikView.machineManagerMainWindow.machineOptions.appendChild(View.sinumerikView.machineManagerMainWindow.machineOptions.defaultMachineButton);
        ...
        View.sinumerikView.machineManagerMainWindow.machineOptions.subroutinesDiv.addButton.addEventListener('click', async function () {
            ...
            if (!canceled) {
                for (let i = 0; i < filePaths.length; i++) {
                    ...
                    View.sinumerikView.machineData.subroutines[View.sinumerikView.machineManagerData.selectedMachine].push(filePaths[i]);
                    saveMachineData();
        ...
        View.sinumerikView.machineManagerMainWindow.machineOptions.snippetsDiv.addButton.addEventListener('click', async function () {
            const {canceled, filePaths} = await dialog.showOpenDialog({properties: ['openDirectory']})
            if (!canceled) {
                for (let i = 0; i < filePaths.length; i++) {
                    ...
                    View.sinumerikView.machineData.snippets[View.sinumerikView.machineManagerData.selectedMachine].push({
                        path: filePaths[i],
                        snippets: await loadSnippet(filePaths[i])
                    });
                    saveMachineData();
...
function loadSubroutinesData(machineName) {
    ...
    for (let i = 0; i < View.sinumerikView.machineData.subroutines[machineName].length; i++) {
        ...
        element.textDiv.className = 'sinumerikMachineTextDiv';
        ...
        element.removeButton.addEventListener('click', function () {
            ...
            if (confirmDialog(`Remove subroutines folder link ${View.sinumerikView.machineData.subroutines[machineName][i]}?`)) {
...
function loadSnippetsData(machineName) {
    ...
    for (let i = 0; i < View.sinumerikView.machineData.snippets[machineName].length; i++) {
        ...
        element.removeButton.addEventListener('click', function () {
            ...
            if (confirmDialog(`Remove snippets folder link ${View.sinumerikView.machineData.snippets[machineName][i].path}?`)) {
...
function toolContainerContent(machineName) {
    ...
    if (!View.sinumerikView.toolContainer.body.firstChild) {
        ...
        const rotate = [315, 0, 45, 270, 0, 90, 225, 180, 135];
        const width = Math.round((View.sinumerikView.right.offsetWidth - 10) / 3);
        for (let i = 0; i < 9; i++) {
            View.sinumerikView.toolContainer.body.elements[i] = document.createElement('div');
            View.sinumerikView.toolContainer.body.elements[i].className = 'sinumerikToolElement';
            ...
            View.sinumerikView.toolContainer.body.elements[i].back_image.className = 'sinumerikToolElementBack';
            ...
            View.sinumerikView.toolContainer.body.elements[i].front_text.className = 'sinumerikToolElementFront';
...
async function createMachineDataFile() {
    try {
        await fsPromises.mkdir(machineDataDir(), {recursive: true});
        View.sinumerikView.machineData = {machines: {}};
        await fsPromises.writeFile(machineDataPath(), JSON.stringify(View.sinumerikView.machineData));
...
function machineManagerCreateFileDiv() {
    ...
    View.sinumerikView.machineManagerCreateFileDiv.className = 'sinumerikCreateFileInfo';
    ...
    View.sinumerikView.machineManagerCreateFileButton.className = 'sinumerikCreateFileButton';
...
function machineManagerCreateMachineMainDiv(event) {
    ...
    if (event == 'createNewMachineSubmitClick') {
        ...
        newMachine.cnc = View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv.select.value
        newMachine.cncOptions = {}
        if (newMachine.cnc.match('FANUC')) {
            newMachine.cncOptions.D_num = View.sinumerikView.machineManagerCreateMachineMainDiv.fanucOptions.querySelector('#D_num').checked
            newMachine.cncOptions.G43 = View.sinumerikView.machineManagerCreateMachineMainDiv.fanucOptions.querySelector('#G43').checked
        }
    ...
    if (event == 'createNewMachineMillSelect') {
        while (View.sinumerikView.machineManagerCreateMachineMainDiv.children[View.sinumerikView.machineManagerCreateMachineMainDiv.children.length - 3].id !== 'machineTypeSelector') {
            View.sinumerikView.machineManagerCreateMachineMainDiv.removeChild(View.sinumerikView.machineManagerCreateMachineMainDiv.children[View.sinumerikView.machineManagerCreateMachineMainDiv.children.length - 3]);
        }
    }
    ...
    if (event == 'createNewMachineLatheSecondCarriageAddButtonClick') {
        ...
        if (!View.sinumerikView.machineManagerCreateMachineMainDiv.carriageDiv.secondCarriageDiv) {
            ...
            View.sinumerikView.machineManagerCreateMachineMainDiv.carriageDiv.secondCarriageDiv.positionSelect.addEventListener('change', function () {
                const type = View.sinumerikView.machineManagerCreateMachineMainDiv.carriageDiv.secondCarriageDiv.positionSelect.value;
    ...
    if (event == 'createNewMachineLatheSecondSpindleAddButtonClick') {
        ...
        if (!View.sinumerikView.machineManagerCreateMachineMainDiv.spindleDiv.secondSpindleDiv) {
            ...
            View.sinumerikView.machineManagerCreateMachineMainDiv.spindleDiv.secondSpindleDiv.inputNameDiv.inputName.className = 'sinumerikInput sinumerikMachineInput native-key-bindings';
            ...
            View.sinumerikView.machineManagerCreateMachineMainDiv.spindleDiv.secondSpindleDiv.inputNumDiv.inputNum.className = 'sinumerikInput sinumerikMachineInput native-key-bindings';
    ...
    if (event == 'createNewMachineLatheSubTypeSelect') {
        ...
        if (!View.sinumerikView.machineManagerCreateMachineMainDiv.carriageDiv) {
            ...
            View.sinumerikView.machineManagerCreateMachineMainDiv.carriageDiv.firstCarriageDiv.positionSelect.addEventListener('change', function () {
                const type = View.sinumerikView.machineManagerCreateMachineMainDiv.carriageDiv.firstCarriageDiv.positionSelect.value;
        ...
        if (!View.sinumerikView.machineManagerCreateMachineMainDiv.spindleDiv) {
            ...
            View.sinumerikView.machineManagerCreateMachineMainDiv.spindleDiv.firstSpindleDiv.inputNameDiv.inputName.className = 'sinumerikInput sinumerikMachineInput native-key-bindings';
            ...
            View.sinumerikView.machineManagerCreateMachineMainDiv.spindleDiv.firstSpindleDiv.inputNumDiv.inputNum.className = 'sinumerikInput sinumerikMachineInput native-key-bindings';
    ...
    if (event == 'createNewMachineLatheSelect') {
        if (!View.sinumerikView.machineManagerCreateMachineMainDiv.subTypeSelectDiv) {
            ...
            View.sinumerikView.machineManagerCreateMachineMainDiv.subTypeSelect.addEventListener('change', function () {
                const type = View.sinumerikView.machineManagerCreateMachineMainDiv.subTypeSelect.value
    ...
    if (event == 'machineManagerNewMachineButtonClick') {
        if (!View.sinumerikView.machineManagerCreateMachineMainDiv) {
            ...
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInputDiv.className = 'sinumerikInputDiv';
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInputLabel = document.createElement('div')
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInputLabel.className = 'sinumerikDiv'
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInputLabel.innerText = 'Please enter machine name'
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInputDiv.appendChild(View.sinumerikView.machineManagerCreateMachineMainDiv.nameInputLabel)
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInputWrapper = document.createElement('div')
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInputWrapper.className = 'sinumerikNameInputWrapper';

            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInput = document.createElement('input');
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInput.className = 'sinumerikInput sinumerikMachineInput native-key-bindings';
            ...
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInput.addEventListener('input', function (e) {
                if (!View.sinumerikView.machineData.machines) return
                const classList = View.sinumerikView.machineManagerCreateMachineMainDiv.nameInput.classList
                const errorClassList = View.sinumerikView.machineManagerCreateMachineMainDiv.nameError.classList
                if (View.sinumerikView.machineData.machines[View.sinumerikView.machineManagerCreateMachineMainDiv.nameInput.value] !== undefined) {
                    View.sinumerikView.machineManagerCreateMachineMainDiv.nameError.innerText = 'Name is already exists'
                    if (classList.contains('sinumerikInput_error')) return
                    classList.add('sinumerikInput_error')
                    if (errorClassList.contains('d_none')) errorClassList.remove('d_none')
                    return
                }
                classList.remove('sinumerikInput_error')
                if (!errorClassList.contains('d_none')) errorClassList.add('d_none')
            });

            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInputWrapper.appendChild(View.sinumerikView.machineManagerCreateMachineMainDiv.nameInput)
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameError = document.createElement('div')
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameError.className = 'sinumerikInput_errorMessage d_none'
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameError.innerText = 'error'
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInputWrapper.appendChild(View.sinumerikView.machineManagerCreateMachineMainDiv.nameError)
            View.sinumerikView.machineManagerCreateMachineMainDiv.nameInputDiv.appendChild(View.sinumerikView.machineManagerCreateMachineMainDiv.nameInputWrapper);
            ...
            View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv = document.createElement('div');
            View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv.nameDiv = document.createElement('div');
            View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv.nameDiv.className = 'sinumerikDiv'
            View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv.nameDiv.innerText = 'cnc'
            View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv.appendChild(View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv.nameDiv)
            View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv.select = document.createElement('select');
            ['SINUMERIK','FANUC (A)','FANUC (B)'].forEach(str => {
                const option = document.createElement('option');
                option.value = str
                option.innerText = str
                View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv.select.appendChild(option)
            })
            View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv.appendChild(View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv.select)
            View.sinumerikView.machineManagerCreateMachineMainDiv.appendChild(View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv)
            View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv.select.addEventListener('change', () => {
                if (View.sinumerikView.machineManagerCreateMachineMainDiv.cncSelectDiv.select.value.match('FANUC')) {
                    View.sinumerikView.machineManagerCreateMachineMainDiv.fanucOptions.style.display = 'flex'
                } else {
                    View.sinumerikView.machineManagerCreateMachineMainDiv.fanucOptions.style.display = 'none'
                }
            })

            View.sinumerikView.machineManagerCreateMachineMainDiv.fanucOptions = document.createElement('div')
            View.sinumerikView.machineManagerCreateMachineMainDiv.fanucOptions.className = 'sinumerikDiv sinumerikBorderDiv sinumerikFanucOptions'

            View.sinumerikView.machineManagerCreateMachineMainDiv.fanucOptions.style.display = 'none'
            addOptions(View.sinumerikView.machineManagerCreateMachineMainDiv.fanucOptions)
            View.sinumerikView.machineManagerCreateMachineMainDiv.appendChild(View.sinumerikView.machineManagerCreateMachineMainDiv.fanucOptions)

            //region type selector
            View.sinumerikView.machineManagerCreateMachineMainDiv.typeSelectDiv = document.createElement('div');
            View.sinumerikView.machineManagerCreateMachineMainDiv.typeSelectDiv.id = 'machineTypeSelector'
            ...
            View.sinumerikView.machineManagerCreateMachineMainDiv.typeSelect.addEventListener('change', function () {
                const type = View.sinumerikView.machineManagerCreateMachineMainDiv.typeSelect.value
            ...
            View.sinumerikView.machineManagerCreateMachineMainDiv.submitButton.addEventListener('click', function () {
                if (!View.sinumerikView.machineManagerCreateMachineMainDiv.nameError.classList.contains('d_none')) {
                    createMachineError('Name is already exists')
                    return
                }
...
export async function saveMachineData(callback) {
    try {
        await fsPromises.writeFile(machineDataPath(), JSON.stringify(View.sinumerikView.machineData));
        loadMachineData(callback);
    } catch (err) {
