...
function machineOptionsHandler(event) {
    ...
    if (event == 'attachMachineButtonClick') {
        ...
        if (extension != 'MPF' && extension != 'MPS') {
            alert('a file extension must be MPF or MPS');
            return;
        }
        if (confirm('This function will add a comment to the top of your file. Do you agree?')) {
            Editor.moveToTop();
            Editor.insertNewline();
            Editor.moveToTop();
            Editor.insertText(';jopa');
        }
    }
