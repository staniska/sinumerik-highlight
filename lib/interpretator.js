        ...
        if (!View.sinumerikView.parseData.jumps.repeat[row] || View.sinumerikView.parseData.jumps.repeat[row].destinationRow == -1) {
            ...
            if (rowText.length == 3) {
                if (rowText[2].match(/[P]\d/) != null) {
                    ...
                    View.sinumerikView.parseData.jumps.repeat[row].maxJumps = mathParse(rowText[2].slice(2), programName);
                } else {
                    return -1;
                }
...
//TODO Сделать массив с jump двумерным с подпрограммами. Иначе возможно совпадение строк в основной програмее и SPF.
    var regEx_IF = /(?<!\w)IF(?!\w)/;
    if (rowText.match(regEx_IF)) {
        ...
        if (!View.sinumerikView.parseData.jumps.ifElseEndif[row]) {
            ...
            if (string.match(/GOTO[BF]?/) == null) {
                View.sinumerikView.parseData.jumps.ifElseEndif[row].Else = searchIfElseEndif(programText, 'ELSE', row, 1);
                View.sinumerikView.parseData.jumps.ifElseEndif[row].Endif = searchIfElseEndif(programText, 'ENDIF', row, 1);
...
//            console.log(str.substring(regExExec[0].length, regExExec[0].length + 1));
    ...
    function checkAssignment(str, programName) {
        //TODO Добавить переменные, определенные в PROC и DEF СРОЧНО БЛЕАТЬ!!!!
...
//    console.log('input string: ' + expression);
    ...
    while (expression.match(/\$AA_IW\[[XYZ]\]/)) {
        var AA_value = View.sinumerikView.parseData.axesPos[expression.match(/\$AA_IW\[[XYZ]\]/)[0].match(/[XYZ]/)];
