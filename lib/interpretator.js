...
import {insertRnd} from './element-insert';
...
function parseDataClear() {
    ...
    View.sinumerikView.parseData.jumps.goto = {};
    View.sinumerikView.parseData.jumps.ifElseEndif = {};
    View.sinumerikView.parseData.jumps.repeat = {};
}
...
function parseRows(programText, programName, variables) {
    ...
    for (let row = rowAfterDefs; row < programText.length; row ++) {
        ...
        if (rowText.trim().length < 2 ||
            ...
            //check IF () GOTO
            var jump = checkIfGoto(rowText, programName, row);
            if (jump >= 0) {
                row = jump - 1;
                continue;
            }
            //check GOTO
            jump = checkGoTo(rowText, programName, row);
            if (jump >= 0) {
                row = jump -1;
                continue;
            }

            jump = checkIfElseEndif(rowText, programName, row);
            if (jump >= 0) {
                row = jump -1;
                continue;
            }

            jump = checkRepeat(rowText, programName, row);
            if (jump >=0) {
                row = jump - 1;
                continue;
            }
...
function checkRepeat(rowText, ProgramName, row) {
    ...
    if (rowText.match(regEx_REPEAT)) {
        ...
        if (!View.sinumerikView.parseData.jumps.repeat[row] || View.sinumerikView.parseData.jumps.repeat[row].destinationRow == -1) {
            View.sinumerikView.parseData.jumps.repeat[row] = {};
            View.sinumerikView.parseData.jumps.repeat[row].jumps = 0;
            View.sinumerikView.parseData.jumps.repeat[row].destinationRow = -1;

            rowText = rowText.replace(/[ ]+/g, ' ').trim().split(' ');
            if (rowText.length == 3) {
                if (rowText[2].match(/[P]\d/) != null) {
                    View.sinumerikView.parseData.jumps.repeat[row].maxJumps = rowText[2].slice(1);
                } else if (rowText[2].match(/[P]=/) != null) {
                    View.sinumerikView.parseData.jumps.repeat[row].maxJumps = mathParse(rowText[2].slice(2));
                } else {
                    return -1;
                }
                if (View.sinumerikView.parseData.jumps.repeat[row].maxJumps == null || View.sinumerikView.parseData.jumps.repeat[row].maxJumps < 0) {
                ...
                for (let i = row - 1; i > 0; i--) {
                    if (Editor.lineTextForBufferRow(i).match(regEx_destName)) {
                        destinationRow = i;
                        View.sinumerikView.parseData.jumps.repeat[row].destinationRow = destinationRow;
                        break;
                    }
                }
                if (destinationRow < 0) {
        ...
        if (View.sinumerikView.parseData.jumps.repeat[row].jumps >= View.sinumerikView.parseData.jumps.repeat[row].maxJumps) {
            return (row + 1);
        }

        View.sinumerikView.parseData.jumps.repeat[row].jumps ++;
        return View.sinumerikView.parseData.jumps.repeat[row].destinationRow;
    }
    return (-1);
}

function checkIfElseEndif(rowText, programName, row) {
    ...
    if (rowText.match(regEx_IF)) {
        ...
        if (!View.sinumerikView.parseData.jumps.ifElseEndif[row]) {
            View.sinumerikView.parseData.jumps.ifElseEndif[row] = {};
            View.sinumerikView.parseData.jumps.ifElseEndif[row].jumps = 0;
            View.sinumerikView.parseData.jumps.ifElseEndif[row].maxJumps = 1000;
            if (View.sinumerikView.savedMaxJumps && View.sinumerikView.savedMaxJumps[row]) {
                View.sinumerikView.parseData.jumps.ifElseEndif[row].maxJumps = View.sinumerikView.savedMaxJumps[row];
            }

            var string = Editor.lineTextForBufferRow(row)
            if (string.match(/GOTO[BF]?/) == null) {
                View.sinumerikView.parseData.jumps.ifElseEndif[row].Else = searchIfElseEndif(Editor, 'ELSE', row, 1);
                View.sinumerikView.parseData.jumps.ifElseEndif[row].Endif = searchIfElseEndif(Editor, 'ENDIF', row, 1);
                if (View.sinumerikView.parseData.jumps.ifElseEndif[row].Endif < 0) {
                    View.sinumerikView.parseData.errors.push({text: `ENDIF not founded.  prog ${programName} row ${row + 1}`, row: row});
                    if (View.sinumerikView.parseData.jumps.ifElseEndif[row].Else > 0) {
                        View.sinumerikView.parseData.jumps.ifElseEndif[row].Endif = View.sinumerikView.parseData.jumps.ifElseEndif[row].Else;
                    } else {
                        View.sinumerikView.parseData.jumps.ifElseEndif[row].Endif = row + 1;
                    }
                }
                if (View.sinumerikView.parseData.jumps.ifElseEndif[row].Else < 0) {
                    View.sinumerikView.parseData.jumps.ifElseEndif[row].Else = View.sinumerikView.parseData.jumps.ifElseEndif[row].Endif;
                }
            }
        }
        // console.log(View.sinumerikView.parseData.jumps.ifElseEndif[row].Else);
        // console.log(View.sinumerikView.parseData.jumps.ifElseEndif[row].Endif);

        if (View.sinumerikView.parseData.jumps.ifElseEndif[row].jumps > View.sinumerikView.parseData.jumps.ifElseEndif[row].maxJumps) {
            if (confirm(`Row ${row + 1}. The number of jumps exceeded ${View.sinumerikView.parseData.jumps.ifElseEndif[row].maxJumps}. Add 1000 more jumps?`)) {
                View.sinumerikView.parseData.jumps.ifElseEndif[row].maxJumps += 1000;
            } else {
                return (View.sinumerikView.parseData.jumps.ifElseEndif[row].Endif + 1);
            }
        }
        ...
        if (condition < 0) {
            if (View.sinumerikView.parseData.jumps.ifElseEndif[row].jumps > 0) {
                View.sinumerikView.parseData.jumps.ifElseEndif[row].jumps = 0;
            }
            View.sinumerikView.parseData.jumps.ifElseEndif[row].jumps --;
            View.sinumerikView.parseData.errors.push({text: `Error in condition "${conditionRow}"  prog ${programName} row ${row + 1}`, row: row});
            return (row + 1);
        }
        if (condition == 0) {
            return (View.sinumerikView.parseData.jumps.ifElseEndif[row].Else + 1);
        }
        if (View.sinumerikView.parseData.jumps.ifElseEndif[row].jumps < 0) {
            View.sinumerikView.parseData.jumps.ifElseEndif[row].jumps = 0;
        }
    ...
    if (rowText.match(regEx_Else)) {
        for (var key in View.sinumerikView.parseData.jumps.ifElseEndif) {
            //console.log(View.sinumerikView.parseData.jumps.ifElseEndif[key]);
            if (row == View.sinumerikView.parseData.jumps.ifElseEndif[key].Else) {
                return (View.sinumerikView.parseData.jumps.ifElseEndif[key].Endif + 1);
            }
        }
    ...
    if (rowText.match(regEx_Endif)) {
        for (var key in View.sinumerikView.parseData.jumps.ifElseEndif) {
            // console.log(View.sinumerikView.parseData.jumps.ifElseEndif[key]);
            if (row == View.sinumerikView.parseData.jumps.ifElseEndif[key].Endif) {
                return (View.sinumerikView.parseData.jumps.ifElseEndif[key].Endif + 1);
            }
        }
...
function checkGoTo(rowText, programName, row) {
    var regEx_GOTO = /(?<!\w)GOTO[B|F]?(?!\w)/
    if (rowText.match(regEx_GOTO)) {
        var Editor = atom.workspace.getActiveTextEditor();

        if (!View.sinumerikView.parseData.jumps.goto[row] || View.sinumerikView.parseData.jumps.goto[row].destinationRow == -1) {
            View.sinumerikView.parseData.jumps.goto[row] = {};
            View.sinumerikView.parseData.jumps.goto[row].jumps = 0;
            View.sinumerikView.parseData.jumps.goto[row].maxJumps = 1000;
            View.sinumerikView.parseData.jumps.goto[row].type = 'GOTO';
            var match = rowText.match(regEx_GOTO);
            var destinationName = rowText.slice(match.index + match[0].length).trim();
            var destinationRow = -1;
            View.sinumerikView.parseData.jumps.goto[row].destinationRow = destinationRow;
            var regEx_destName = new RegExp(`(?<!\\w)${destinationName}\\:(?!\\w)`);
            // console.log(match);
            if (match[0] == 'GOTOB') {
                for (let i = row - 1; i > 0; i --) {
        ...
        if (View.sinumerikView.parseData.jumps.goto[row].jumps > View.sinumerikView.parseData.jumps.goto[row].maxJumps) {
        ...
        View.sinumerikView.parseData.jumps.goto[row].jumps ++;
        return View.sinumerikView.parseData.jumps.goto[row].destinationRow;

    }
    return -1;
}

function checkIfGoto (rowText, programName, row) {
    var regEx_IF = /(?<!\w)IF(?!\w)/;
    var regEx_GOTO = /(?<!\w)GOTO[B|F]?(?!\w)/
    if (rowText.match(regEx_IF) && rowText.match(regEx_GOTO)) {
        ...
        if (!View.sinumerikView.parseData.jumps.ifGoto[row] || View.sinumerikView.parseData.jumps.ifGoto[row].destinationRow == -1) {
            ...
            if (View.sinumerikView.savedMaxJumps && View.sinumerikView.savedMaxJumps[row]) {
                View.sinumerikView.parseData.jumps.ifGoto[row].maxJumps = View.sinumerikView.savedMaxJumps[row];
            }
        ...
        if (View.sinumerikView.parseData.jumps.ifGoto[row].jumps > View.sinumerikView.parseData.jumps.ifGoto[row].maxJumps) {
            if (confirm(`Row ${row + 1}. The number of jumps exceeded ${View.sinumerikView.parseData.jumps.ifGoto[row].maxJumps}. Doubled max jumps?`)) {
                View.sinumerikView.parseData.jumps.ifGoto[row].maxJumps *= 2;
                console.log(View.sinumerikView.parseData.jumps.ifGoto[row].maxJumps);
                if (!View.sinumerikView.savedMaxJumps) {
                    View.sinumerikView.savedMaxJumps = {};
                }
                View.sinumerikView.savedMaxJumps[row] = View.sinumerikView.parseData.jumps.ifGoto[row].maxJumps;
            } else {
                return (row + 1);
            }
        }
...
function generateCanvasPrimitives(primitives, programName, row) {
    ...
    if (pole.type) {
        ...
        View.sinumerikView.parseData.pole.AP = 0;
        View.sinumerikView.parseData.pole.RP = 0;
        return;
    }
    //endregion
    ...
    if (Object.keys(move).length) {
        ...
        if (View.sinumerikView.parseData.moveGroup.match(/G2|G3/)) {
            ...
            if (!circleError) {
                ...
                if (View.sinumerikView.parseData.prevMove.length) {
                    // console.log(View.sinumerikView.parseData.prevMove);
                    if (View.sinumerikView.parseData.prevMove[0].type == 'G1') {
                ...
                for (let i = 0; i < pointsNum; i++) {
                    ...
                    thisPoint[circleAxes[2]] = canvasElement[`${circleAxes[2]}_start`] + (canvasElement[circleAxes[2]] - canvasElement[`${circleAxes[2]}_start`]) * ((i+1)/pointsNum);
        ...
        if (View.sinumerikView.parseData.moveGroup.match(/G0|G1|G33/)) {
            ...
            if (View.sinumerikView.parseData.prevMove.length) {
                ...
                if (View.sinumerikView.parseData.prevMove[0].type.match(/G[1-3]/)) {
                    var elements = insertRnd(View.sinumerikView.parseData.prevMove[0], canvasElement, programName);
                    if (View.sinumerikView.parseData.prevMove[0].type == 'G1') {
                        View.sinumerikView.parseData.canvas.push(View.sinumerikView.parseData.prevMove[0]);
                    }
                    View.sinumerikView.parseData.prevMove.shift();
        ...
        while (true) {
            ...
            if (splitRow[i].match(/(?<!\w)AP\=/)) {
                ...
                if (mathVal != null) {
                    ...
                    View.sinumerikView.parseData.errors.push({text:`Right part of AP assignment error. prog ${programName} row ${progRowNum + 1}`, row: progRowNum})
                }
                break;
            }
            ...
            if (splitRow[i].match(/(?<!\w)RP\=/)) {
                ...
                if (mathVal != null) {
                    ...
                    View.sinumerikView.parseData.errors.push({text:`Right part of RP assignment error. prog ${programName} row ${progRowNum + 1}`, row: progRowNum})
                }
                break;
            }
            //endregion
            ...
            if (splitRow[i].match(/(?<!\w)ANG|CHR|CHF|RNDM(?=[\=])/)) {
                primitives[0].operators[i].type = 'Move modificator';
                primitives[0].operators[i].value = splitRow[i].match(/(?<!\w)ANG|CHR|CHF|RNDM(?=[\=])/)[0];
                break;
            }
            // endregion
            ...
            if (splitRow[i].match(/(?<!\w)RND\=/)) {
                ...
                var mathVal = mathParse(splitRow[i].slice(4));
                if (mathVal != null) {
                    primitives[0].operators[i].value = mathVal;
                } else {
