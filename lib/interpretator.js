...
//Пока не открывать. На Github нет snippets
// import {generateSnippetsArray} from "./snippets";

const myMath = new sinumerikMath();

class CanvasElementsArray extends Array {
    push(element) {
        if (View.sinumerikView.parseData.offn != 0) {
    ...
    if (splitRow[0] == 'CALL') {
        ...
        View.sinumerikView.parseData.subroutines.forEach(function (subroutine) {
            if (subroutineName == subroutine.name) {
                ...
                parseRows(programmText, recursProgName, subroutineVariables);
