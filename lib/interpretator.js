...
function checkCondition(expression, programName) {
    ...
    expression.split(/[&|]+/).forEach((exp_part) => {
        ...
        if (expression.match('=')) {
            // TODO !!!!
            let leftPart = expression.substring(0, expression.match(/[\!\>\<\=]+/).index)
            let startIndex = leftPart.lastIndexOf('(') + 1
            // console.log(leftPart)
            expression = expression.replace(
                expression.substring(startIndex, expression.match(/[\!\>\<\=]+/).index),
                parseFloat(expression.substring(startIndex, expression.match(/[\!\>\<\=]+/).index)).toFixed(4)
            )
            if (expression.match(/(\|\|)|(\&\&)/)) {
                const firstComparsion = expression.substring(0, expression.match(/(\|\|)|(\&\&)/).index)
                let secondComparsion = expression.substring(expression.match(/(\|\|)|(\&\&)/).index)

                if (secondComparsion.match('=')) {
                    leftPart = secondComparsion.substring(0, expression.match(/[\!\>\<\=]+/).index)
                    startIndex = 3
                    if (leftPart.lastIndexOf('(')) {
                        startIndex = leftPart.lastIndexOf('(') + 1
                    }
                    secondComparsion = secondComparsion.replace(
                        secondComparsion.substring(startIndex, secondComparsion.match(/[\!\>\<\=]+/).index),
                        parseFloat(secondComparsion.substring(startIndex, secondComparsion.match(/[\!\>\<\=]+/).index)).toFixed(4)
                    )
                    expression = firstComparsion + secondComparsion
                }
                console.log(expression)

                // leftPart = expression.substring(, expression.match(/[\!\>\<\=]+/).index)
            }
        }
    });
    ...
    if (Object.keys(move).length) {
        ...
        if (View.sinumerikView.parseData.moveGroup == 'G2' ||
            ...
            if (move.circle_CR != undefined) {
                ...
                // console.log(coordsInFrame);
                if (Math.abs(distance - 2 * Math.abs(move.circle_CR) < 1e-4)) {
                    View.sinumerikView.parseData.errors.push({
