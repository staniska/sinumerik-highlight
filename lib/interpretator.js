...
import {normalizeFileName, readProgramLines, readShapeLines, extractBoundingContourBlock} from "./utils";
...
export async function progParser(lastLineNum) {
    ...
    if (!View.sinumerikView.parseData ||
        ...
        View.sinumerikView.parseData.contourElementsMeta = {}
        View.sinumerikView.parseData.subroutineMeta = {}
...
function parseDataErrorsDisplay() {
    ...
    const hasErrors = View.sinumerikView.parseData.errors.length > 0;
    if (hasErrors) {
        stringDiv.innerText = `ERR (${View.sinumerikView.parseData.errors.length}).  Last: ${View.sinumerikView.parseData.errors[View.sinumerikView.parseData.errors.length - 1].text}`;
    } else {
        stringDiv.innerText = 'PARSE OK';
    }
    stringDiv.classList.toggle('sinumerikSLDStringDiv--error', hasErrors);
    stringDiv.classList.toggle('sinumerikSLDStringDiv--ok', !hasErrors);
...
function parseDataClear(programName) {
    ...
    // Reset shape/call contour buckets so a re-parse (e.g. toggling the
    // contour/blank duplicate checkbox) starts fresh. Without this, every
    // re-parse appends to existing buckets — geometry accumulates with extra
    // G0 approach lines. Bounding-contour buckets are persistent (their
    // contents come from a separate mini-parse, not from the main pass).
    const oldContourElements = View.sinumerikView.parseData.contourElements || {};
    const oldContourElementsMeta = View.sinumerikView.parseData.contourElementsMeta || {};
    const freshContourElements = {};
    const freshContourElementsMeta = {};
    Object.keys(oldContourElements).forEach(k => {
        if (k.startsWith('BOUNDING_')) {
            freshContourElements[k] = oldContourElements[k];
            if (oldContourElementsMeta[k]) freshContourElementsMeta[k] = oldContourElementsMeta[k];
        }
    });
    View.sinumerikView.parseData.contourElements = freshContourElements;
    View.sinumerikView.parseData.contourElementsMeta = freshContourElementsMeta;
    View.sinumerikView.parseData.subroutineMeta = {};
...
async function parseRows(programText, programName, variables, firstCall, bucketKey) {
    // bucketKey === null is explicit "discard pushes" (e.g. subroutine call into
    // a shape already filled by shape-extraction). bucketKey === undefined falls
    // back to programName (default top-level/shape-extraction behaviour).
    if (bucketKey === undefined) bucketKey = programName
    View.sinumerikView.parseData.currentBucket = bucketKey
    if (bucketKey && !View.sinumerikView.parseData.contourElements[bucketKey]) {
        View.sinumerikView.parseData.contourElements[bucketKey] = []
    }
    ...
    let rowAfterDefs = 0;
    // Bounding-contour mini-parse pre-seeds variables[programName] with the
    // captured PROC scope; don't wipe it here.
    if (View.sinumerikView.boundingMiniParseScope !== programName) {
    ...
    // Bounding-contour snapshot: PROC params are bound, body has not run,
    // so axesPos still reflects the call site. Snapshot now and bail out
    // of further captures if visited more than once (loop/recursion).
    const bcReq = View.sinumerikView.boundingContourRequest;
    if (bcReq?.captureNext) {
        bcReq.captureNext = false;
        bcReq.visited = (bcReq.visited || 0) + 1;
        if (bcReq.visited > 1) {
            bcReq.error = 'Subroutine call is visited more than once (loop or recursion).';
            bcReq.snapshot = null;
        } else {
            const pd = View.sinumerikView.parseData;
            bcReq.snapshot = {
                subProgName: programName,
                axesPos: { ...pd.axesPos },
                diamon: pd.diamon,
                diam90: pd.diam90,
                plane: pd.plane,
                variables: JSON.parse(JSON.stringify(pd.variables[programName] || {})),
            };
        }
    }
    ...
    //region main_prog
    if (firstCall) {
        // Capture the generation at the start of firstCall processing.
        // After every await we verify it hasn't been superseded by a newer
        // drawChanges call — if it has, we bail out so stale elements don't
        // corrupt the canvas that the new parse is building.
        const firstCallGeneration = _parseGeneration
        ...
        if (View.sinumerikView.parseData.subroutines.filter(subroutine => {
            ...
            const blankProgText = await readShapeLines(blank.path);
            if (_parseGeneration !== firstCallGeneration) return
            View.sinumerikView.parseData.calledSubroutines.add(blank.path)
            const blankProgName = normalizeFileName(blank.path.split(/[\\/]/).pop());
            View.sinumerikView.parseData.contourElementsMeta = View.sinumerikView.parseData.contourElementsMeta || {};
            if (!View.sinumerikView.parseData.contourElementsMeta[blankProgName]) {
                View.sinumerikView.parseData.contourElementsMeta[blankProgName] = {
                    basename: blank.name,
                    path: blank.path,
                    source: 'shape',
                };
            }
            const prevBucketBlank = View.sinumerikView.parseData.currentBucket;
            await parseRows(blankProgText, blankProgName, null, false)
            View.sinumerikView.parseData.currentBucket = prevBucketBlank;
            if (_parseGeneration !== firstCallGeneration) return
        ...
        if (View.sinumerikView.parseData.subroutines.filter(subroutine => {
            ...
            const contourProgText = await readShapeLines(contour.path);
            if (_parseGeneration !== firstCallGeneration) return
            View.sinumerikView.parseData.calledSubroutines.add(contour.path)
            const contourProgName = normalizeFileName(contour.path.split(/[\\/]/).pop());
            View.sinumerikView.parseData.contourElementsMeta = View.sinumerikView.parseData.contourElementsMeta || {};
            if (!View.sinumerikView.parseData.contourElementsMeta[contourProgName]) {
                View.sinumerikView.parseData.contourElementsMeta[contourProgName] = {
                    basename: contour.name,
                    path: contour.path,
                    source: 'shape',
                };
            }
            const prevBucketContour = View.sinumerikView.parseData.currentBucket;
            await parseRows(contourProgText, contourProgName, null, false)
            View.sinumerikView.parseData.currentBucket = prevBucketContour;
            if (_parseGeneration !== firstCallGeneration) return
            View.sinumerikView.parseData.canvas.forEach(element => {
                // if (element.type !== 'pause') {
...
// Run the bounding-contour block from a subroutine source in an isolated
// scope using the snapshot captured during the main parse. Produces
// parseData.contourElements[bucket] with the resulting primitives.
// Returns the bucket key on success, or false if the block is missing
// or the snapshot is unavailable.
export async function runBoundingContourParse(req) {
    if (!req || !req.snapshot || !req.subPath) return false

    const lines = await readProgramLines(req.subPath)
    const block = extractBoundingContourBlock(lines)
    if (!block) {
        req.error = `No bounding-contour block in ${req.subName}.`
        return false
    }

    const pd = View.sinumerikView.parseData
    const bucket = `BOUNDING_${req.subName}_${req.callRow + 1}`
    ...
    const requestBackup = View.sinumerikView.boundingContourRequest

    pd.canvas  = new CanvasElementsArray()
    pd.axesPos = { ...req.snapshot.axesPos }
    if (req.snapshot.diamon !== undefined) pd.diamon = req.snapshot.diamon
    if (req.snapshot.diam90 !== undefined) pd.diam90 = req.snapshot.diam90
    if (req.snapshot.plane  !== undefined) pd.plane  = req.snapshot.plane

    pd.variables[bucket] = JSON.parse(JSON.stringify(req.snapshot.variables))

    // Disable snapshot hook for the mini-parse — nested calls inside the
    // bounding block must not try to snapshot again.
    View.sinumerikView.boundingContourRequest = null

    if (!View.sinumerikView.boundingContourMeta) {
        View.sinumerikView.boundingContourMeta = {}
    }
    View.sinumerikView.boundingContourMeta[bucket] = {
        subName:    req.subName,
        callRow:    req.callRow,
        callerFile: req.callerFile,
    }

    // Tell parseRows not to wipe variables[bucket] on entry — we already
    // seeded it with the captured PROC scope above.
    View.sinumerikView.boundingMiniParseScope = bucket

    try {
        await parseRows(block.body, bucket, [], false)
    } finally {
        pd.canvas  = backup.canvas
        pd.axesPos = backup.axesPos
        pd.diamon  = backup.diamon
        pd.diam90  = backup.diam90
        pd.plane   = backup.plane
        delete pd.variables[bucket]
        if (pd.jumps) delete pd.jumps[bucket]
        if (pd.mcall) delete pd.mcall[bucket]
        View.sinumerikView.boundingContourRequest = requestBackup
        View.sinumerikView.boundingMiniParseScope = null
    }
