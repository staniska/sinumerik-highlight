...
export async function progParser(lastLineNum) {
    ...
    parseDataClear(normalizeFileName(Editor.getTitle()));
...
function parseDataClear(programName) {
    ...
    if (View.sinumerikView.programmData[normalizeFileName(atom.workspace.getActiveTextEditor().getTitle())].machine.machineType === 'Lathe') {
...
async function parseRows(programText, programName, variables, firstCall) {
    ...
    if (firstCall) {
        ...
        if (View.sinumerikView.parseData.subroutines.filter(subroutine => {
            ...
            const blankProgText = (await fsPromises.readFile(`${blank.path}`, 'utf8')).split('\n');
        ...
        if (View.sinumerikView.parseData.subroutines.filter(subroutine => {
            ...
            const contourProgText = (await fsPromises.readFile(`${contour.path}`, 'utf8')).split('\n');
    ...
    for (let row = rowAfterDefs; row < programText.length; row++) {
        ...
        if (rowText.trim().length < 2 ||
            ...
            const primitives = await generatePrimitives(rowText, programName, row, programText)
            ...
            if (primitives) {
                ...
                for (let i = 0; i < primitives.length; i++) {
                    ...
                    if (View.sinumerikView.parseData.moveGroup === 'transform') {
                        if (!primitives[i].operators.filter((o) => o.type === 'moveGroup').length) {
                            primitives[i].operators.push({
                                type: 'moveGroup',
                    ...
                    for (const iter in primitives[i].operators) {

                        const operator = primitives[i].operators[iter];
                        const machine = View.sinumerikView.programmData[normalizeFileName(atom.workspace.getActiveTextEditor().getTitle())].machine
...
const addY_for_C_rot = (primitives) => {
...
function generateCirclePrimitives(programName) {
    ...
    let pointsNum = Math.round((circleRadius_prev * Math.PI) * (Math.abs(arcAng_prev) / 180));
    if (pointsNum < 30) {
        pointsNum = 30;
    }
    if (pointsNum > 300) {
...
async function generatePrimitives(rowText, programName, progRowNum, programText) {
    const primitives = [];
    ...
    if (splitRow[0] == 'CALL') {
        ...
        for (const subroutine of View.sinumerikView.parseData.subroutines) {
            if (subroutineName == subroutine.name) {
                ...
                await parseRows(programmText, recursProgName, subroutineVariables, false);
...
//        console.log(frameRow);
    ...
    function checkCoordinates(str) {
        ...
        if (spindleAxis != '' && spindleAxis != 'C') {
            const regEx = new RegExp(`(?<!\\w)((${spindleAxis})|([XYZABC]))(?=[\\d\\=\\-\\+])`);
        }
