...
function checkFor (rowText, programName, programText, row) {
    ...
    if (rowText.match(regEx_For)[0] === 'FOR') {
        ...
        if (!View.sinumerikView.parseData.jumps[programName].for[row]) {
            ...
            //Переменная для FOR
            let varString = rowText.substring(4).split('=')[0].trim()
...
function checkCondition(expression, programName) {
   ...
   expression.split(/[&|]+/).forEach((exp_part) => {
        ...
        //remove parentheses from exp_part
        while (exp_part[0] == '(' && exp_part[exp_part.length - 1] == ')') {
            exp_part = exp_part.slice(1, -1);
        }

        // console.log('condition part without parentheses: ', exp_part);

        var exp_left = exp_part.split(/[!><=]+/)[0];
        var exp_right = exp_part.split( /[!><=]+/)[1];
        // console.log(exp_part.match(/[\>\<\=]+/)[0])
        // console.log(mathParse(exp_left, programName))
        var exp_part_value = eval(`${mathParse(exp_left, programName)} ${exp_part.match(/[\!\>\<\=]+/)[0]} ${mathParse(exp_right, programName)}`);
