        ...
        if (operator.type == 'coordinate') {
            if (!operator.subtype) {
                ...
                if (operator.subtype == 'IC') {
                    ...
                    //Мегакостыль для станков с IC на сторону
                    if (View.sinumerikView.programmData[programName].machine.incrementMove &&
                        View.sinumerikView.programmData[programName].machine.machineType == 'Lathe' &&
                        operator.name == 'X' &&
                        View.sinumerikView.programmData[programName].machine.incrementMove == 'Radius' &&
                        View.sinumerikView.parseData.diamon) {
                        operator.value *= 2;
                    };
    ...
    for (let i = 0; i < splitRow.length; i ++) {
        ...
        while (true) {
            ...
            if (splitRow[i].match(/(?<!\w)AP\=/)) {
                ...
                //TODO Добавить обработку IC для AP и RP;
                var mathVal = mathParse(splitRow[i].slice(3), programName);
                if (mathVal != null) {
                    primitives[0].operators[i].value = mathVal;
                } else {
