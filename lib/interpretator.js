...
class CanvasElementsArray extends Array {
    push(element) {
        ...
        //TODO Переделать mirror на этап до формирования canvas с заменами G2/3 G41/42
...
export function progParser(lastLineNum) {
    ...
    //TODO Сделать ввод dirPath с локальной машины, если работаем через teletype
    var files = 0
    if (dirPath[0] !== '@') {
        files = fs.readdirSync(dirPath);
    } else {
        let path
        if (confirm(`Teletype detected ${filename}.\n Do you want to select a local directory containing additional files? \n
    ...
    if (rowText.match(regEx_While)) {
        if (rowText.match(regEx_While)[0] === 'WHILE') {
            ...
            if (!View.sinumerikView.parseData.jumps[programName].while[row]) {
                ...
                let while_level = 0
                for (let i = row + 1; i < programText.length; i++) {
                    if (programText[i].match(/(?<!\w)WHILE(?=\W|$)/)) {
                        while_level++
                    }
                    if (programText[i].match(/ENDWHILE(?=\W|$)/) && while_level === 0) {
                        View.sinumerikView.parseData.jumps[programName].while[row].endwhile = i;
                        break;
                    }
                    if (programText[i].match(/ENDWHILE(?=\W|$)/)) {
                        while_level--
                    }
                }

                if (View.sinumerikView.parseData.jumps[programName].while[row].endwhile === undefined) {
                    View.sinumerikView.parseData.errors.push({text: `ENDWHILE not found. prog ${programName} row ${row + 1}`, row: row});
                    return -1
                }
            }
        ...
        if (rowText.match(regEx_While)[0] === 'ENDWHILE') {
            ...
            if (View.sinumerikView.parseData.jumps[programName].while[while_row].jumps > View.sinumerikView.parseData.jumps[programName].while[while_row].maxJumps) {
                if (confirm(`The number of jumps at WHILE (row${View.sinumerikView.parseData.jumps[programName].while[while_row].while})
...
function checkFor (rowText, programName, programText, row) {
    ...
    if (rowText.match(regEx_For)[0] === 'FOR') {
        ...
        if (!View.sinumerikView.parseData.jumps[programName].for[row]) {
                ...
                if (varString.match(/R[0-9]+/) &&
                    ...
                    try {
                        View.sinumerikView.parseData.variables.firstChannelVariables[varString] = {
                            name: varString,
                            type: 'real',
                            value: mathParse(rowText.split('=')[1].split('TO')[0].trim(), programName)
                        }
                    } catch (e) {
                        View.sinumerikView.parseData.errors.push({text: `Error in FOR . prog ${programName} row ${row + 1}`, row: row});
                    }
            ...
            if (View.sinumerikView.parseData.jumps[programName].for[row].endfor === undefined) {
                View.sinumerikView.parseData.errors.push({text: `ENDFOR not found. prog ${programName} row ${row + 1}`, row: row});
                return -1
            }
        }
    ...
    if (rowText.match(regEx_For)[0] === 'ENDFOR') {
        ...
        if (View.sinumerikView.parseData.jumps[programName].for[for_row].jumps > View.sinumerikView.parseData.jumps[programName].for[for_row].maxJumps) {
            if (confirm(`The number of jumps at FOR (row${View.sinumerikView.parseData.jumps[programName].for[for_row].for})
...
function checkCondition(expression, programName) {
    ...
    expression.split(/[&|]+/).forEach((exp_part) => {
        // console.log(exp_part);
        //insert '(' & ')' in string for normal work of match
        exp_part = '(' + exp_part.trim() + ')';

        while (exp_part.match(/\(/g).length > exp_part.match(/\)/g).length) {
            exp_part = exp_part.slice(1);
        }

        while (exp_part.match(/\(/g).length < exp_part.match(/\)/g).length) {
            exp_part = exp_part.slice(0, -1);
        }

        var exp_left = exp_part.split(/[!><=]+/)[0];
        var exp_right = exp_part.split(/[!><=]+/)[1];

        //TODO Зачам-то здесь я пытался удалить лишние скобки. Разобраться по возможности
        // while (exp_left[0] == '(' && exp_left[exp_left.length - 1] == ')') {
        //     exp_left = exp_left.slice(1, -1);
        // }
        // while (exp_right[0] == '(' && exp_right[exp_right.length - 1] == ')') {
        //     exp_right = exp_right.slice(1, -1);
        // }
        console.log(exp_left, exp_part.match(/[\!\>\<\=]+/)[0], exp_right)

        var exp_part_value = eval(`${mathParse(exp_left, programName)}
        ${exp_part.match(/[\!\>\<\=]+/)[0]}
        ${mathParse(exp_right, programName)}`);
        // console.log(`${mathParse(exp_left, programName)} ${exp_part.match(/[\!\>\<\=]+/)[0]} ${mathParse(exp_right, programName)}`)
        expression = expression.replace(exp_part, exp_part_value.toString());
    });
//    console.log('condition intermediate result: ', expression);
    var value = 0;
...
function generateCanvasPrimitives(primitives, programName, row) {
    ...
    //TODO Проверить работу полюса вместе с фреймами. Похоже при TRANS они съезжают
    //region pole
    var pole = {};
    //check G111-112 (Pole)
...
function generatePrimitives(rowText, programName, progRowNum) {
    ...
    if (splitRow[0] == 'CALL') {
        ...
        subroutineVariables = subroutineVariables.map((variable => {
            // console.log(variable)
            return mathParse(variable, programName) != null ? mathParse(variable, programName).toString() : variable
        }))
...
//        console.log(frameRow);
    ...
    if (regEx.exec(programRow)) {
        ...
        let mathVal = mathParse(regEx.exec(programRow)[0].slice(5), programName)
        primitives[0].operators[0].value = mathVal !== null ? mathVal : 0;
        if (mathVal === null) {
            View.sinumerikView.parseData.errors.push({text:`Right part of OFFN assignment error. prog ${programName} row ${progRowNum + 1}`, row: progRowNum})
        }
    ...
    //region SPCO[NF]
    regEx = /(?<!\w)SPCO[NF](?!\w)/;
    if (regEx.exec(programRow) && View.sinumerikView.singleLineDebugHelpDiv.C_As_Rot.input.checked) {
...
//            console.log(str.substring(regExExec[0].length, regExExec[0].length + 1));
    ...
    function checkAssignment(str, programName) {
        ...
        if (returnVal.name != undefined &&
            View.sinumerikView.parseData.variables[programName][returnVal.name] &&
            View.sinumerikView.parseData.variables[programName][returnVal.name].type.substring(0,6) == 'string') {
...
export function mathParse(expression, programName) {
    // console.log(expression);
    // console.log(View.sinumerikView.parseData.variables);
    if (Object.keys(View.sinumerikView.parseData.variables.firstChannelVariables).length) {
        for (var variable in View.sinumerikView.parseData.variables.firstChannelVariables) {
