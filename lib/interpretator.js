...
function checkJump (rowText, programName, row) {
    ...
    if (rowText.match(regEx_IF) && rowText.match(regEx_GOTO)) {
        ...
        if (!View.sinumerikView.parseData.jumps.ifGoto[row]) {
    ...
    if (Object.keys(move).length) {
        ...
        if (View.sinumerikView.parseData.moveGroup.match(/G2|G3/)) {
            ...
            if (!circleError) {
                ...
                var vectorEndAng = myMath.atan2(move[circleAxes[1]] - move[circleCenterAxes[1]], move[circleAxes[0]] - move[circleCenterAxes[0]]);
