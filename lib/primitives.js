...
import {parseAutoComment} from './inner-comment';
...
export const addY_for_C_rot = (primitives) => {
    ...
    const sldFile = View.sinumerikView.parseData.filename
        ?? View.sinumerikView.singleLineDebugData?.filename
        ?? atom.workspace.getActiveTextEditor()?.getPath()
    const machine = View.sinumerikView.programmData[sldFile]?.machine
...
export async function generatePrimitives(rowText, programName, progRowNum, programText, parseRowsFn) {
    ...
    if (splitRow[0] == 'CALL') {
        ...
        // Dedup: skip drawing path of a subroutine that the caller already
        // declared as its blank/contour shape. The caller can be either:
        //   - the main program (programmData[programName] from the editor file's
        //     auto-comment),
        //   - or another subroutine (subroutineMeta[programName] from the sub's
        //     own auto-comment, populated in stage 2 when the sub is first entered).
        const callerContourName = View.sinumerikView.parseData.subroutineMeta?.[programName]?.contour?.name
            ?? View.sinumerikView.programmData[programName]?.contour?.name;
        const callerBlankName = View.sinumerikView.parseData.subroutineMeta?.[programName]?.blank?.name
            ?? View.sinumerikView.programmData[programName]?.blank?.name;
        if (callerContourName === subroutineName &&
            !View.sinumerikView.singleLineDebugContourDiv.contourDuplicate.checked) {
            primitive.operators[0] = {};
            return primitive;
        }
        if (callerBlankName === subroutineName &&
            !View.sinumerikView.singleLineDebugBlankDiv.blankDuplicate.checked) {
            primitive.operators[0] = {};
            return primitive;
        }
        ...
        for (const subroutine of View.sinumerikView.parseData.subroutines) {
            if (subroutineName == subroutine.name) {
                const programmText = await readProgramLines(subroutine.path);
                View.sinumerikView.parseData.calledSubroutines.add(subroutine.path)
                const programmName = normalizeFileName(subroutine.path.split(/[\\/]/).pop());
                // _r suffix protects variable scope from recursion/repeat —
                // but the contourElements bucket must NOT get _r, otherwise the
                // panel sees ghost entries. See CLAUDE.md.
                ...
                const bucketKey = programmName;
                // Pre-seed subroutineMeta with the subroutine's own auto-comment
                // (blank/contour names) so the dedup logic at line 741 can look
                // it up via subroutineMeta[programName] for nested calls.
                View.sinumerikView.parseData.subroutineMeta = View.sinumerikView.parseData.subroutineMeta || {};
                if (!View.sinumerikView.parseData.subroutineMeta[recursProgName]) {
                    View.sinumerikView.parseData.subroutineMeta[recursProgName] = parseAutoComment(programmText.join('\n'));
                }
                View.sinumerikView.parseData.contourElementsMeta = View.sinumerikView.parseData.contourElementsMeta || {};
                const prevBucket = View.sinumerikView.parseData.currentBucket;
                const prevMeta = prevBucket && View.sinumerikView.parseData.contourElementsMeta[prevBucket];
                // CONTOUR_MIRR.MPF pattern: a 3-line wrapper (MIRROR / CONTOUR / MIRROR)
                // around the real shape file. The wrapper has no geometry of its own
                // — nested CONTOUR call must push into the wrapper's bucket so the
                // panel entry that matches auto-comment (CONTOUR_MIRR) is populated
                // with the (already-transformed) geometry.
                const inShapeWrapper = prevMeta?.source === 'shape';
                const existingMeta = View.sinumerikView.parseData.contourElementsMeta[bucketKey];
                // Main body calling a shape that was already filled by firstCall —
                // don't duplicate the geometry.
                const skipBucketPush = !inShapeWrapper && existingMeta?.source === 'shape';
                if (!inShapeWrapper && !existingMeta) {
                    View.sinumerikView.parseData.contourElementsMeta[bucketKey] = {
                        basename: subroutineName,
                        path: subroutine.path,
                        source: 'call',
                    };
                }

                // Bounding-contour: signal parseRows to snapshot variables and
                // axesPos after this sub's PROC params are bound but before its
                // body runs. See CLAUDE.md bounding-contour section.
                const bcReq = View.sinumerikView.boundingContourRequest;
                if (bcReq && !bcReq.error
                    && bcReq.callerProgName === programName
                    && bcReq.callRow === progRowNum
                    && bcReq.subName === subroutineName) {
                    bcReq.captureNext = true;
                    bcReq.subPath = subroutine.path;
                    bcReq.subProgName = recursProgName;
                }

                const canvasLengthBefore = View.sinumerikView.parseData.canvas.length;
                const nestedBucketKey = inShapeWrapper
                    ? prevBucket
                    : (skipBucketPush ? null : bucketKey);
                await parseRowsFn(programmText, recursProgName, subroutineVariables, false, nestedBucketKey);
                View.sinumerikView.parseData.currentBucket = prevBucket;
                // Stamp every element added by the callee with this call-site entry.
                // Prepend so that by the time we bubble up to main the array reads
                // outermost-first: [{file:main, row:M1}, {file:A, row:A1}, ...]
                // mainRow is kept for backward-compat with the Canvas 2D renderer.
                const callerPath = _resolveCallerPath(
                    programName, View.sinumerikView.parseData
                );
...
// Resolve the real filesystem path for a given parseRows programName.
// Main program: programName === parseData.filename, return it directly.
// Subroutine: strip _r suffixes and look up in parseData.subroutines.
function _resolveCallerPath(programName, parseData) {
    if (programName === parseData.filename) return parseData.filename
    const baseName = programName.replace(/(_r)+$/, '')
    const sub = (parseData.subroutines ?? []).find(s =>
        normalizeFileName(s.path.split(/[\\/]/).pop()) === baseName
    )
    return sub?.path ?? programName
}
