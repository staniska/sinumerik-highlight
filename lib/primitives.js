    ...
    if (splitRow[0] == 'CALL') {
        ...
        for (const subroutine of View.sinumerikView.parseData.subroutines) {
            if (subroutineName == subroutine.name) {
                ...
                // Always overwrite mainRow — not just when undefined. Without this,
                // elements from B in a chain main(M1)→A(A1)→B get mainRow=A1 (set
                // by the A→B return) and the M1 assignment is then skipped at the
                // A→main return because mainRow is already set. The WebGL click
                // handler uses mainRow as a line number in the main program, so it
                // would jump to A1 instead of M1.
