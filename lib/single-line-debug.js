...
import {init2DRenderer, draw2D, destroy2DRenderer, resize2DRenderer, is2DRendererActive} from "./sld_view/renderer2d.js";
            ...
            if (View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.checked) {
                View.sinumerikView.parseData.canvas.forEach((element) => {
                    if (element.type != 'msg' || element.type != 'pause') {
                        ...
                        if (element_length > 1) {
                            let element_devider = 1;
                            if (element_length > 20) {
                                element_devider = Math.round(element_length / 20)
                            }
                            const little_element_nums = Math.round(element_length / element_devider);
                            const little_element = JSON.parse(JSON.stringify(element));
