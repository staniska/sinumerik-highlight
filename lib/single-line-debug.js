...
function generateSLDComponents() {
    ...
    View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheckDiv.innerText = 'Slow debug';
    ...
    View.sinumerikView.singleLineDebugInfoDiv.appendChild(View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck);
...
function displayCanvasElements(ctx) {
    ...
    if (!View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.checked) {
        ...
        var element_num = 0;
//TODO добавить деление длинных линий на части

        var axes_names = ['X','Y','Z'];
        ctx_timeout = setTimeout(function draw() {
                var element_length = 0;
                axes_names.forEach(function (axis) {
                    element_length += (View.sinumerikView.parseData.canvas[element_num][axis] - (View.sinumerikView.parseData.canvas[element_num][`${axis}_start`])) ** 2;
                });
                element_length = Math.sqrt(element_length);

                if (element_length > 1) {
                    var little_element_length = element_length / Math.round(element_length);
                    for (let i = 0; i < Math.round(element_length); i++) {
                        //TODO сделать много маленьких элементов
                    }
                } else {
                    ctx_draw(View.sinumerikView.parseData.canvas[element_num]);
                }

                element_num++;
                // console.log(View.sinumerikView.parseData.canvas.length);
                if (element_num < View.sinumerikView.parseData.canvas.length) {
                    ctx_timeout = setTimeout(draw, 0.1);
                }
            }, 0.2);
    }
...
function drawAxes(ctx) {
    ...
    //TODO Заменить крестик на координатные линейки
