...
// Scroll the SLD editor (and subroutine editor if open) to the line of el.
// Resolves sourceFile the same way subroutinePopup does: real paths start
// with / or a drive letter; otherwise match against parseData.subroutines.
function _sldScrollToElement(el) {
    ...
    if (mainEditor) {
        mainEditor.setCursorBufferPosition([mainRow, 0])
    ...
    if (el.mainRow !== undefined && el.sourceFile) {
        ...
        if (subEditor) {
            subEditor.setCursorBufferPosition([el.row, 0])
