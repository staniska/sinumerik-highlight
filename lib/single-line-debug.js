...
import {normalizeFileName, subdivideElement} from "./utils";
...
import {select2DView, select3DView, draw3DScene} from "./3d_view/view.js";
import {init2DRenderer, draw2D, draw2DUpTo, destroy2DRenderer, resize2DRenderer, is2DRendererActive, getWebGLWrapper} from "./sld_view/renderer2d.js";
...
//Global timer
let ctx_timeout;
let _animDisposables = null
let _decorationDisposables = null
let _skipSldScroll = false
//Массив для элементов заготовки
let blank = []
let contour = []

function _startAnimSubscriptions() {
    _stopAnimSubscriptions()
    const sourceEditor = atom.workspace.getActiveTextEditor()
    if (!sourceEditor) return
    _animDisposables = [
        sourceEditor.onDidChange(() => finishSlowDebugAnimation()),
        atom.workspace.onDidChangeActiveTextEditor(() => finishSlowDebugAnimation())
    ]
}

function _stopAnimSubscriptions() {
    _animDisposables?.forEach(d => d.dispose())
    _animDisposables = null
}

function _stopDecorationCursorWatch() {
    _decorationDisposables?.forEach(d => d.dispose())
    _decorationDisposables = null
}

function _startDecorationCursorWatch() {
    _stopDecorationCursorWatch()
    const editors = [_sldMainDecoration?.editor, _sldSubDecoration?.editor].filter(Boolean)
    if (!editors.length) return
    _decorationDisposables = editors.map(editor =>
        editor.onDidChangeCursorPosition(() => _destroySldDecorations())
    )
}

export function finishSlowDebugAnimation() {
    const infoDiv = View.sinumerikView.singleLineDebugInfoDiv
    const slowOrPause = infoDiv?.slowDebugCheck?.checked || infoDiv?.pauseCheck?.level > 0
    if (!slowOrPause) return
    const elements = View.sinumerikView.parseData?.elements
    if (!elements?.length) return
    const drawn = View.sinumerikView.parseData.drawnElementsNums ?? 0
    if (drawn >= elements.length) return

    clearTimeout(ctx_timeout)
    _stopAnimSubscriptions()
    View.sinumerikView.parseData.drawnElementsNums = elements.length

    _skipSldScroll = true
    changeCanvas()
    _skipSldScroll = false

    // Set decoration on the last element without scrolling, then watch for
    // cursor movement so the highlight clears when the user starts editing.
    const lastEl = elements[elements.length - 1]
    _sldScrollToElement(lastEl, false)
    _startDecorationCursorWatch()
}

//TODO помимо изменения имени файла. проверять еще и путь

export async function singleLineDebug(event) {
//    console.log('SLD ' + event);
    ...
    if (event.match(/[XYZ]+PlaneButtonClick/)) {
        await changePlane(event);
    }

    if (event === 'stringParseDetailsButtonClick') {
        const was3D    = View.sinumerikView.singleLineDebugViewDiv.plane2D3D.innerText === '2D';
        const wasWebGL = View.sinumerikView.singleLineDebugData.useWebGL;
        if (was3D) select2DView();
        const mainWindow = View.sinumerikView.singleLineDebugMainWindow;
        if (wasWebGL) {
            // WebGL wrapper is managed by renderer2d — destroy removes it from DOM.
            destroy2DRenderer();
        } else {
            mainWindow.removeChild(View.sinumerikView.singleLineDebugCanvas);
        }
        mainWindow.appendChild(View.sinumerikView.singleLineDebugStringParseDetailsDiv);
        ...
        View.sinumerikView.closeDetailsButton.dataset.returnTo3d  = was3D    ? '1' : '';
        View.sinumerikView.closeDetailsButton.dataset.returnToWebGL = wasWebGL ? '1' : '';
    }
    if (event === 'stringParseDetailsCloseButtonClick') {
        const mainWindow  = View.sinumerikView.singleLineDebugMainWindow;
        const returnWebGL = View.sinumerikView.closeDetailsButton.dataset.returnToWebGL === '1';
        mainWindow.removeChild(View.sinumerikView.singleLineDebugStringParseDetailsDiv);
        if (returnWebGL) {
            // Re-initialise WebGL renderer into the same container and redraw.
        ...
        if (View.sinumerikView.closeDetailsButton.dataset.returnTo3d) {
            select3DView();
        }
    }
    ...
    if (event === 'changeCanvasPrimitives') {
        ...
        if (View.sinumerikView.singleLineDebugViewDiv.plane2D3D.innerText === '2D') {
            ...
            const pauseLevel = View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.level
            if (slowDebug || pauseLevel > 0) {
                _3DSlowDebug()
            } else {
                select3DView()
            }
...
function _getSldEditor() {
    const pinned = View.sinumerikView.sldPinnedFile
    if (pinned) return atom.workspace.getTextEditors().find(e => e.getPath() === pinned)
    return atom.workspace.getActiveTextEditor()
}

async function checkChanges(event) {

    const Editor = _getSldEditor();
    if (!Editor) return;
    ...
    if (!View.sinumerikView.singleLineDebugData ||
        ...
        if (View.sinumerikView.singleLineDebugThree) select2DView()
        generateSLDData();
        event = '';
    }
    ...
    if (View.sinumerikView.singleLineDebugErrorDiv.textContent == '') {
        if (View.sinumerikView.singleLineDebugData.machine.machineType == 'Lathe') {
            await changePlane('XZPlaneButtonClick');
        } else {
            await changePlane('XYPlaneButtonClick');
        }
    }
}
...
function generateSLDData() {
    ...
    if (!Editor.getTitle().split('.').pop().toUpperCase().match(/MPF|SPF/)) {
...
function errorDiv(text) {
    ...
    if (!View.sinumerikView.singleLineDebugErrorDiv) {
        ...
        View.sinumerikView.singleLineDebugErrorDiv.className = 'sinumerikDiv sinumerikSLDErrorDiv';
    }
    if (text === '') {
        View.sinumerikView.singleLineDebugErrorDiv.innerText = text;
        if (View.sinumerikView.singleLineDebugErrorDiv.parentElement) {
            View.sinumerikView.singleLineControlsFootContainer.removeChild(View.sinumerikView.singleLineDebugErrorDiv);
        }
        View.sinumerikView.singleLineDebugControlsContainer.appendChild(View.sinumerikView.singleLineDebugLeftFootDiv);
        View.sinumerikView.singleLineDebugControlsContainer.appendChild(View.sinumerikView.singleLineDebugMiddleFootDiv);
        View.sinumerikView.singleLineDebugControlsContainer.appendChild(View.sinumerikView.singleLineDebugRightFootDiv);
        View.sinumerikView.singleLineDebugControlsContainer.appendChild(View.sinumerikView.singleLineDebugRightFootDiv2);
    } else {
        View.sinumerikView.singleLineDebugErrorDiv.innerText = text;
        ...
        View.sinumerikView.singleLineDebugControlsContainer.appendChild(View.sinumerikView.singleLineDebugErrorDiv);

        if (View.sinumerikView.singleLineDebugLeftFootDiv.parentElement) {
            View.sinumerikView.singleLineDebugControlsContainer.removeChild(View.sinumerikView.singleLineDebugLeftFootDiv);
            View.sinumerikView.singleLineDebugControlsContainer.removeChild(View.sinumerikView.singleLineDebugMiddleFootDiv);
            View.sinumerikView.singleLineDebugControlsContainer.removeChild(View.sinumerikView.singleLineDebugRightFootDiv);
            View.sinumerikView.singleLineDebugControlsContainer.removeChild(View.sinumerikView.singleLineDebugRightFootDiv2);
        }
    }
}

async function changePlane(event) {
...
function generateSLDComponents() {
    ...
    View.sinumerikView.singleLineDebugCanvas.addEventListener('wheel', event => {
        ...
        if (newRange !== newRange || newRange < CANVAS_RANGE_MIN || newRange > CANVAS_RANGE_MAX) return;
    ...
    View.sinumerikView.singleLineDebugData.useWebGL =
        localStorage.getItem('sinumerik-sld-renderer') === 'webgl'

    //Three div's for data, controls & axes canvas;
    View.sinumerikView.singleLineDebugLeftFootDiv = document.createElement('div');
    View.sinumerikView.singleLineDebugLeftFootDiv.className = 'sinumerikDiv sinumerikBorderDiv sinumerikSLDFootDiv';
    View.sinumerikView.singleLineDebugMiddleFootDiv = document.createElement('div');
    View.sinumerikView.singleLineDebugMiddleFootDiv.className = 'sinumerikDiv sinumerikBorderDiv sinumerikSLDFootDiv';
    View.sinumerikView.singleLineDebugRightFootDiv = document.createElement('div');
    View.sinumerikView.singleLineDebugRightFootDiv.className = 'sinumerikDiv sinumerikBorderDiv sinumerikSLDFootDiv';

    View.sinumerikView.singleLineDebugRightFootDiv2 = document.createElement('div');
    View.sinumerikView.singleLineDebugRightFootDiv2.className = 'sinumerikDiv sinumerikBorderDiv sinumerikSLDFootDiv';

    View.sinumerikView.singleLineDebugFootMain = create_element([],View.sinumerikView.singleLineDebugFootContainer)
    View.sinumerikView.singleLineDebugFootProgressBar = progressBar(View.sinumerikView.singleLineDebugFootMain)
    View.sinumerikView.singleLineDebugFootProgressBar.style.display = 'none'

    View.sinumerikView.singleLineDebugControlsContainer = create_element(['sinumerikSingleLineDebugFoot'],View.sinumerikView.singleLineDebugFootMain)
    ...
    View.sinumerikView.singleLineDebugParseStringDiv.className = 'sinumerikSLDParseStringDiv';
    View.sinumerikView.singleLineDebugParseStringDiv.stringDiv = document.createElement('div');
    View.sinumerikView.singleLineDebugParseStringDiv.stringDiv.className = 'sinumerikSLDStringDiv';
    ...
    View.sinumerikView.singleLineDebugParseStringButton.className = 'sinumerikButton icon-info sinumerikSLDButtonRight';
    ...
    const rendererToggleWrap = document.createElement('div');
    rendererToggleWrap.className = 'sinumerikSLDRendererToggle';
    rendererToggleWrap.title = 'Switch between Canvas 2D and WebGL renderer';

    const canvasOpt = document.createElement('button');
    canvasOpt.className = 'sinumerikSLDRendererOption' +
        (View.sinumerikView.singleLineDebugData.useWebGL ? '' : ' sinumerikSLDRendererOption--active');
    canvasOpt.innerText = 'Canvas';
    canvasOpt.addEventListener('click', () => {
        if (View.sinumerikView.singleLineDebugData.useWebGL) switchRenderer();
    });

    const webglOpt = document.createElement('button');
    webglOpt.className = 'sinumerikSLDRendererOption' +
        (View.sinumerikView.singleLineDebugData.useWebGL ? ' sinumerikSLDRendererOption--active' : '');
    webglOpt.innerText = 'WebGL';
    webglOpt.addEventListener('click', () => {
        if (!View.sinumerikView.singleLineDebugData.useWebGL) switchRenderer();
    });

    rendererToggleWrap.appendChild(canvasOpt);
    rendererToggleWrap.appendChild(webglOpt);
    View.sinumerikView.rendererToggleButton = rendererToggleWrap;
    View.sinumerikView.singleLineDebugParseStringDiv.prepend(rendererToggleWrap);

    View.sinumerikView.closeDetailsButton = document.createElement('button');
    View.sinumerikView.closeDetailsButton.className = 'sinumerikButton icon-x sinumerikSLDButtonRight';
    ...
    View.sinumerikView.sldPinButton = create_element(
        ['sinumerikButton', 'icon-pin', 'sinumerikSLDPinButton'],
        View.sinumerikView.singleLineDebugParseStringDiv,
        'button'
    )
    View.sinumerikView.sldPinButton.title = 'Pin current file to SLD'
    View.sinumerikView.sldPinButton.addEventListener('click', () => View.sinumerikView.toggleSldPin())

    View.sinumerikView.singleLineDebugControlsContainer.insertBefore(
        View.sinumerikView.singleLineDebugParseStringDiv,
        View.sinumerikView.singleLineDebugControlsContainer.firstChild
    );

    View.sinumerikView.singleLineDebugStringParseDetailsDiv = document.createElement('div');
    View.sinumerikView.singleLineDebugStringParseDetailsDiv.className = 'sinumerikSLDDetailsDiv';
    ...
    View.sinumerikView.singleLineDebugInfoDiv.machineNameDiv.className = 'sinumerikDiv sinumerikSLDMachineName';
    ...
    View.sinumerikView.singleLineDebugInfoDiv.realTimeDebugCheckDiv.className = 'sinumerikSLDCheckDiv';
    ...
    View.sinumerikView.singleLineDebugInfoDiv.realTimeDebugCheck.className = 'sinumerikSLDCheckbox';
    ...
    View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheckDiv.className = 'sinumerikSLDCheckDivFlex';
    ...
    View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.className = 'sinumerikSLDCheckbox';
    View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.addEventListener('change', function (e) {
        View.sinumerikView.singleLineDebugFootProgressBar.style.display = e.target.checked ? 'flex': 'none'
        if (View.sinumerikView.singleLineDebugInfoDiv.realTimeDebugCheck.checked && View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.checked) {
        ...
        if (!e.target.checked) {
            clearTimeout(ctx_timeout)
            _stopAnimSubscriptions()
            _destroySldDecorations()
            View.sinumerikView.parseData.drawnElementsNums = null
            changeCanvas()
        }
    });
    ...
    View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheckMult.className = 'sinumerikSLDSelect';
    ...
    View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheckText.className = 'sinumerikSLDAutoLabel';
    ...
    View.sinumerikView.singleLineDebugInfoDiv.pauseDiv.className = 'sinumerikSLDPauseDiv';
    View.sinumerikView.singleLineDebugInfoDiv.pauseCheck = document.createElement('button');
    View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.className = 'sinumerikSLDPauseButton';
    ...
    function changePauseBackground() {
        const jo = 5 + (View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.level) * 27 - 5;
        View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.style.backgroundImage =
            `linear-gradient(to right, var(--pause-mid) ${jo}%, var(--pause-on) ${jo}%, var(--pause-off) ${6 + jo}%)`;
    }
    ...
    View.sinumerikView.singleLineDebugHelpDiv.className = 'sinumerikDiv sinumerikSLDHelpDiv';
    ...
    View.sinumerikView.singleLineDebugHelpDiv.G40div.textDiv.className = 'sinumerikSLDHelpText';
    ...
    View.sinumerikView.singleLineDebugHelpDiv.G40div.lineDiv.className = 'sinumerikSLDColorLine sinumerikSLDColorLine--g40';
    ...
    View.sinumerikView.singleLineDebugHelpDiv.G41div.textDiv.className = 'sinumerikSLDHelpText';
    ...
    View.sinumerikView.singleLineDebugHelpDiv.G41div.lineDiv.className = 'sinumerikSLDColorLine sinumerikSLDColorLine--g41';
    ...
    View.sinumerikView.singleLineDebugHelpDiv.G42div.textDiv.className = 'sinumerikSLDHelpText';
    ...
    View.sinumerikView.singleLineDebugHelpDiv.G42div.lineDiv.className = 'sinumerikSLDColorLine sinumerikSLDColorLine--g42';
    ...
    View.sinumerikView.singleLineDebugHelpDiv.CompMath.className = 'sinumerikSLDCompMath';

    View.sinumerikView.singleLineDebugHelpDiv.CompMathText = document.createElement('div');
    View.sinumerikView.singleLineDebugHelpDiv.CompMathText.className = 'sinumerikSLDCompMathText';
...
//region C_As_Rot
    ...
    View.sinumerikView.singleLineDebugHelpDiv.C_As_Rot.textDiv.className = 'sinumerikSLDInlineLabel';
    ...
    View.sinumerikView.singleLineDebugHelpDiv.C_As_Rot.input.className = 'sinumerikSLDSmallCheckbox';
...
//region blank
    ...
    View.sinumerikView.singleLineDebugBlankDiv.className = 'sinumerikSLDRowFlex';
    View.sinumerikView.singleLineDebugBlankDiv.textDiv = document.createElement("div")
    View.sinumerikView.singleLineDebugBlankDiv.textDiv.className = 'sinumerikSLDFieldLabel';
    ...
    View.sinumerikView.singleLineDebugBlankDiv.input.className = 'sinumerikInput sinumerikSLDFieldInput native-key-bindings'
    ...
    View.sinumerikView.singleLineDebugBlankDiv.blankDuplicate = create_element([], View.sinumerikView.singleLineDebugBlankDiv, 'input')
    const blankDuplicate = View.sinumerikView.singleLineDebugBlankDiv.blankDuplicate
    blankDuplicate.type = 'checkbox'
    blankDuplicate.className = 'sinumerikSLDCenteredCheckbox';
    blankDuplicate.title = 'Duplicate drawing the same blank from program'
    blankDuplicate.addEventListener('change', () => {View.redraw()})
...
//region contour
    ...
    View.sinumerikView.singleLineDebugContourDiv.className = 'sinumerikSLDRowFlex';
    View.sinumerikView.singleLineDebugContourDiv.textDiv = document.createElement("div")
    View.sinumerikView.singleLineDebugContourDiv.textDiv.className = 'sinumerikSLDFieldLabel';
    ...
    View.sinumerikView.singleLineDebugContourDiv.input.className = 'sinumerikInput sinumerikSLDFieldInput native-key-bindings'
    ...
    View.sinumerikView.singleLineDebugContourDiv.contourDuplicate = create_element([], View.sinumerikView.singleLineDebugContourDiv, 'input')
    const contourDuplicate = View.sinumerikView.singleLineDebugContourDiv.contourDuplicate
    contourDuplicate.type = 'checkbox'
    contourDuplicate.className = 'sinumerikSLDCenteredCheckbox';
    contourDuplicate.title = 'Duplicate drawing the same contour from program'
    contourDuplicate.addEventListener('change', () => {View.redraw()})
...
//region first resize
    ...
    View.sinumerikView.singleLineDebugStringParseDetailsDiv.style.height = `${Math.round((editorHeight - 40) * mainHeightFactor) - 5}px`;
//endregion
...
export function resizeSLDComponents(panelWidth) {
    ...
    const mainHeightFactor = 0.8;
    const oldCanvasWidth = View.sinumerikView.singleLineDebugCanvas.width;
    ...
    if (View.sinumerikView.singleLineDebugData.useWebGL) {
        resize2DRenderer(Math.round(panelWidth) - 5, Math.round((editorHeight - 40) * mainHeightFactor) - 5)
    }
    View.sinumerikView.singleLineDebugStringParseDetailsDiv.style.width = View.sinumerikView.singleLineDebugCanvas.width;
    View.sinumerikView.singleLineDebugStringParseDetailsDiv.style.height = `${Math.round((editorHeight - 40) * mainHeightFactor) - 5}px`;

    if (View.sinumerikView.singleLineDebugData.canvasTransform.scale && oldCanvasWidth > 0) {
        View.sinumerikView.singleLineDebugData.canvasRange *=
            View.sinumerikView.singleLineDebugCanvas.width / oldCanvasWidth;
        View.sinumerikView.singleLineDebugData.canvasHeightFactor =
            View.sinumerikView.singleLineDebugCanvas.height / View.sinumerikView.singleLineDebugCanvas.width;
        transformByRange();
        changeCanvas();
    }
}
...
function switchRenderer() {
    const data = View.sinumerikView.singleLineDebugData;
    data.useWebGL = !data.useWebGL;
    localStorage.setItem('sinumerik-sld-renderer', data.useWebGL ? 'webgl' : 'canvas');

    const [canvasOpt, webglOpt] = View.sinumerikView.rendererToggleButton.children;
    canvasOpt.classList.toggle('sinumerikSLDRendererOption--active', !data.useWebGL);
    webglOpt.classList.toggle('sinumerikSLDRendererOption--active', data.useWebGL);

    const mainWindow = View.sinumerikView.singleLineDebugMainWindow;
    const canvas2D = View.sinumerikView.singleLineDebugCanvas;

    if (data.useWebGL) {
        // Ensure data.canvasRange/canvasCentrPoint are real numbers — they
        // start as {} from generateSLDComponents and only get populated
        // when Canvas 2D path runs canvasRange(). Without this the WebGL
        // camera frustum is NaN and nothing renders on first switch.
        if (typeof data.canvasRange !== 'number' || !data.canvasCentrPoint) {
            canvasRange();
        }
        if (mainWindow.contains(canvas2D)) {
            mainWindow.removeChild(canvas2D);
        }
        init2DRenderer(mainWindow);
        draw2D();
    } else {
        destroy2DRenderer();
        if (!mainWindow.contains(canvas2D)) {
            mainWindow.insertBefore(canvas2D, mainWindow.firstChild);
        }
        canvasRange();
    }
}
...
function canvasRange(event) {
    const Editor = _getSldEditor();
    if (!Editor) return;
    const filename = normalizeFileName(Editor.getPath());
    ...
    const scaleMoveFactor = Number((View.sinumerikView.singleLineDebugData.canvasRange / 20).toPrecision(1));


    if (event === 'scaleMinusButtonClick') {
        View.sinumerikView.singleLineDebugData.canvasRange = Math.min(
    ...
    if (event === 'scalePlusButtonClick') {
        View.sinumerikView.singleLineDebugData.canvasRange = Math.max(
            View.sinumerikView.singleLineDebugData.canvasRange - scaleMoveFactor,
            CANVAS_RANGE_MIN
        );
    }
...
export function transformByRange() {
...
let _sldMainDecoration = null
let _sldSubDecoration  = null

function _destroySldDecorations() {
    _sldMainDecoration?.marker?.destroy(); _sldMainDecoration = null
    _sldSubDecoration?.marker?.destroy();  _sldSubDecoration  = null
    _stopDecorationCursorWatch()
}

// Highlight the active slow-debug line in the editor(s) using a marker
// decoration so the user's cursor is not disturbed.
function _sldScrollToElement(el, scroll = true) {
    if (_skipSldScroll) return
    if (!el || el.type === 'msg' || el.type === 'pause') return
    _destroySldDecorations()

    const mainRow = el.mainRow ?? el.row
    const mainEditor = _getSldEditor()
    if (mainEditor) {
        const marker = mainEditor.markBufferRange([[mainRow, 0], [mainRow, Infinity]], {invalidate: 'never'})
        mainEditor.decorateMarker(marker, {type: 'line',        class: 'sld-active-line'})
        mainEditor.decorateMarker(marker, {type: 'line-number', class: 'sld-active-line'})
        _sldMainDecoration = {marker, editor: mainEditor}
        if (scroll) mainEditor.scrollToBufferPosition([mainRow, 0], {center: true})
    }

    if (el.mainRow !== undefined && el.sourceFile) {
        let subPath = el.sourceFile
        if (subPath[0] !== '/' && subPath[1] !== ':') {
            const baseName = subPath.replace(/(_r)+$/, '')
            const sub = (View.sinumerikView.parseData?.subroutines ?? [])
                .find(s => s.path.slice(s.path.lastIndexOf('/') + 1)
                    .replace('.', '_').toUpperCase() === baseName)
            if (sub) subPath = sub.path
        }
        const subEditor = atom.workspace.getTextEditors().find(e => e.getPath() === subPath)
        if (subEditor) {
            const marker = subEditor.markBufferRange([[el.row, 0], [el.row, Infinity]], {invalidate: 'never'})
            subEditor.decorateMarker(marker, {type: 'line',        class: 'sld-active-line'})
            subEditor.decorateMarker(marker, {type: 'line-number', class: 'sld-active-line'})
            _sldSubDecoration = {marker, editor: subEditor}
            if (scroll) subEditor.scrollToBufferPosition([el.row, 0], {center: true})
        }
    }
}

function _webGLSlowDebug() {
    clearTimeout(ctx_timeout)

    const rawCanvas = View.sinumerikView.parseData?.canvas ?? []

    // Build little-element array the same way Canvas 2D does — subdivide
    // long G1/G0 moves so trajectory appears progressively rather than
    // jumping in one frame. Reuse parseData.elements if already built
    // (resume after pause).
    if (!View.sinumerikView.parseData.drawnElementsNums) {
        let elements = []
        if (View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.checked) {
            rawCanvas.forEach(element => {
                if (element.type === 'msg' || element.type === 'pause') {
                    elements.push(element)
                } else {
                    subdivideElement(element).forEach(sub => elements.push(sub))
                }
            })
        } else {
            elements = rawCanvas
        }
        View.sinumerikView.parseData.elements = elements
    }

    const elements = View.sinumerikView.parseData.elements ?? rawCanvas
    const progress = View.sinumerikView.singleLineDebugFootProgressBar.querySelector('progress')
    progress.max   = elements.length
    progress.value = View.sinumerikView.parseData.drawnElementsNums ?? 0

    draw2DUpTo(View.sinumerikView.parseData.drawnElementsNums ?? 0, elements)
    _sldScrollToElement(elements[(View.sinumerikView.parseData.drawnElementsNums ?? 1) - 1])

    if (View.sinumerikView.singleLineDebugData.progressBarDebug) return
    if (!elements.length) return

    _startAnimSubscriptions()

    let idx     = View.sinumerikView.parseData.drawnElementsNums ?? 0
    let pause   = 0
    let timeout = 5
    const timeout_K = View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.checked ? 1 : 0

    ctx_timeout = setTimeout(function step() {
        ...
        if (pause !== 1) {
            if (View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.checked ||
                ...
                if (el.type === 'msg' && level >= 3) { msg_div(el); pause = 1 }
                if (el.type === 'pause') {
                    if (el.value === 'M0' || (el.value === 'M1' && level > 1)) { msg_div(el); pause = 1 }
                    if (el.value === 'The end' &&
                        !View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.checked &&
                        level === 4) {
                        el.value = el.programName + '. The end.'
                        msg_div(el)
                        pause = 1
                    }
                }
            ...
            if (!pause) {
                idx++
                View.sinumerikView.parseData.drawnElementsNums = idx

                timeout = timeout_K * (el.type === 'G0' ? 5 : 100)
                if (timeout > 0 && el.type === 'G1') {
                    const len = Math.sqrt(['X', 'Y', 'Z'].reduce(
                        (sum, ax) => sum + ((el[ax] - el[`${ax}_start`]) ** 2), 0
                    ))
                    timeout = len < 0.3 ? 0
                        : timeout * View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheckMult.value * len
                }

                draw2DUpTo(idx, elements)
                _sldScrollToElement(el)
            }
        } else {
            timeout = 200
            if (!View.sinumerikView.msgDiv.parentElement) {
                ...
                idx++
                View.sinumerikView.parseData.drawnElementsNums = idx
                _sldScrollToElement(elements[idx - 1])
            }
        }

        if (idx < elements.length && !(idx === elements.length - 1 && pause)) {
            ctx_timeout = setTimeout(step, timeout)
...
function _3DSlowDebug() {
    ...
    draw3DScene(View.sinumerikView.parseData.drawnElementsNums ?? 0, elements)
    _sldScrollToElement(elements[(View.sinumerikView.parseData.drawnElementsNums ?? 1) - 1])
...
function extractBlankAndContour() {
    const parseData = View.sinumerikView.parseData;
    if (!parseData?.canvas?.length) return;

    const extract = (kind) => {
        if (parseData.canvas[0]?.[kind] === undefined) return;
        let result = [];
        while (parseData.canvas.length && parseData.canvas[0]?.[kind] !== undefined) {
            result.push(parseData.canvas.shift());
        }

        result = result.filter(el => el.type.match(/G[0-3]/));

        if (result.length > 1) {
            // Build a connector segment so the polygon closes: start of element[1]
            // (the first real edge after the approach move) joins to the end of
            // the last edge. element[0] was the approach and gets dropped.
            const axes_names = ['X', 'Y', 'Z'];
            const connector = {...result[result.length - 1]};
            axes_names.forEach(axis => {
                connector[axis] = result[1][`${axis}_start`];
                connector[`${axis}_start`] = result[result.length - 1][axis];
            });
            result.shift();
            result.push(connector);
        }
        parseData[kind] = result;
        if (kind === 'blank')   blank   = result;
        if (kind === 'contour') contour = result;
    };

    extract('blank');
    extract('contour');
    syncContourEditShapes();
}

// Mirror parseData.blank/contour into contourEditData so the contourEdit tab
// can render shapes regardless of which SLD renderer (Canvas 2D / WebGL / 3D)
// is active. Previously this lived inside displayCanvasElements (Canvas 2D
// only), so switching to contourEdit from WebGL showed an empty canvas.
function syncContourEditShapes() {
    if (!View.sinumerikView.contourEditData) return;
    const toShape = (el) => ({
        start: {X: el.X_start, Y: el.Y_start, Z: el.Z_start},
        end:   {X: el.X,       Y: el.Y,       Z: el.Z},
    });
    const parseData = View.sinumerikView.parseData;
    if (parseData?.blank?.length) {
        View.sinumerikView.contourEditData.blank = parseData.blank.map(toShape);
    }
    if (parseData?.contour?.length) {
        View.sinumerikView.contourEditData.contour = parseData.contour.map(toShape);
    }
}

export function changeCanvas() {
    // Pull blank / contour out of parseData.canvas before either renderer
    // touches it. The original Canvas-2D-only extraction lived inside
    // displayCanvasElements and mutated parseData.canvas with .shift() —
    // when WebGL renders first, that path is skipped and blank/contour
    // segments are drawn as a regular trajectory.
    extractBlankAndContour();

    // If the 3D view is active, neither 2D path should draw. Crucially,
    // resizePanel() calls changeCanvas() after select3DView() destroyed the
    // WebGL renderer; without this guard the useWebGL lazy-init below would
    // recreate the WebGL renderer right on top of the 3D canvas.
    if (View.sinumerikView.singleLineDebugThree) {
        // Drive the 3D slow-debug loop from here so the progress bar's
        // changeCanvas() calls (scrubbing / resume / resize) work the same as
        // for the 2D renderers. _3DSlowDebug respects progressBarDebug and
        // drawnElementsNums, so it is safe to call from any of these contexts.
        const infoDiv = View.sinumerikView.singleLineDebugInfoDiv
        const slowOrPause = infoDiv?.slowDebugCheck?.checked || infoDiv?.pauseCheck?.level > 0
        if (slowOrPause || View.sinumerikView.singleLineDebugData?.progressBarDebug) {
            _3DSlowDebug()
        } else {
            draw3DScene(Infinity)
        }
        return
    }

    if (View.sinumerikView.singleLineDebugData?.useWebGL) {
        // First-time activation (useWebGL persisted in localStorage but the
        // renderer was never inited because generateSLDComponents always
        // creates the Canvas 2D element). Lazy-init here.
    ...
    const ctx = View.sinumerikView.singleLineDebugCanvas.getContext("2d");
...
function displayCanvasElements(ctx) {
    ...
    if (!View.sinumerikView.parseData.canvas.length &&
        !View.sinumerikView.parseData.blank?.length &&
        !View.sinumerikView.parseData.contour?.length) {
        return
    }

    // blank/contour are already extracted from parseData.canvas by
    // extractBlankAndContour() in changeCanvas(); sync the module-level
    // refs so save_blank()/save_contour() can still clear them later.
    blank   = View.sinumerikView.parseData.blank   ?? blank;
    contour = View.sinumerikView.parseData.contour ?? contour;

    const axes = View.sinumerikView.singleLineDebugData.CanvasAxes.axes;
    const factor = [1, -1];
    ...
    if (!View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.checked && View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.level === 0) {
            ...
            if (View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.checked) {
                View.sinumerikView.parseData.canvas.forEach((element) => {
                    if (element.type === 'msg' || element.type === 'pause') {
                        ...
                        subdivideElement(element).forEach(sub => array_of_little_elements.push(sub));
                    }
                })
            } else {
        ...
        const progress = View.sinumerikView.singleLineDebugFootProgressBar.querySelector('progress')
        progress.max = View.sinumerikView.parseData.elements.length

        if (View.sinumerikView.parseData.drawnElementsNums) {
            progress.value = View.sinumerikView.parseData.drawnElementsNums
        ...
        _sldScrollToElement(View.sinumerikView.parseData.elements[(View.sinumerikView.parseData.drawnElementsNums ?? 1) - 1])

        if (!View.sinumerikView.singleLineDebugData.progressBarDebug) {
            _startAnimSubscriptions()
            ctx_timeout = setTimeout(function draw() {
                ...
                progress.value++
                if (pause !== 1) {
                    const el = array_of_little_elements[idx]
                    pause = ctx_draw(el, true);
                    ...
                    if (timeout > 0 && el.type == 'G1') {
                        ...
                        let length_of_little_element = Math.sqrt(axes.reduce(
                            (total, axis) => total + ((el[axis] - el[`${axis}_start`]) ** 2), 0
                        ))
    ...
    function ctx_draw(element, msg) {
        ...
        const selectionFactor = View.sinumerikView.selection ?
            (View.sinumerikView.selectionContains(element.mainRow ?? element.row) ? 2 : 0.5) : 1
        ...
        if (element.type === 'transform') ctx.lineWidth *= 0.7
    ...
    function ctx_draw_fill(array, type) {
        ...
        const blankColor   = css.getPropertyValue('--blank-color').trim()   || 'lightgray'
        const contourColor = css.getPropertyValue('--contour-color').trim() || 'darkgray'
        ctx.fillStyle = (type === 'blank' ? blankColor : contourColor)
...
export function canvasElementColor(type, toolRadiusCompensation) {
    if (type === 'transform') {
        return 'black'
    }
...
export function drawAxes(ctx) {
    const css = getComputedStyle(ctx.canvas);
    const axesColor     = css.getPropertyValue('--axes-color').trim()      || 'rgba(47, 79, 79, 0.5)';
    const axesTextColor = css.getPropertyValue('--axes-text-color').trim() || 'rgba(47, 79, 79, 0.8)';
    const arrowSide = View.sinumerikView.singleLineDebugData.canvasRange / 40;
    const reverseFactor = [1, -1];
    View.sinumerikView.singleLineDebugData.canvasHeightFactor = View.sinumerikView.singleLineDebugCanvas.height / View.sinumerikView.singleLineDebugCanvas.width;

    const t = View.sinumerikView.singleLineDebugData.canvasTransform;
    const fillText = (text, worldX, worldY) => {
        ctx.save();
        ctx.setTransform(1, 0, 0, 1, 0, 0);
        ctx.fillStyle = axesTextColor;
        ctx.font = '16px Verdana';
        ctx.fillText(text, worldX * t.scale + t.e, worldY * t.scale + t.f);
        ctx.restore();
    };
    ...
    //
    let base_for_scale_pipki = 5 * scale_length * Math.round(View.sinumerikView.singleLineDebugData.canvasCentrPoint[0] / (5 * scale_length));

    //палочки на горизонтальной линейке
    let pipki_num = Math.round(View.sinumerikView.singleLineDebugData.canvasRange / scale_length);
    for (let i = 0; i < pipki_num; i++) {
        let five_factor;
        if (Math.round((base_for_scale_pipki + (i - Math.round(pipki_num / 2)) * scale_length) / scale_length) % 5 === 0) {
            fillText(
...
export { _sldScrollToElement as sldScrollToElement, _startDecorationCursorWatch as startDecorationCursorWatch }