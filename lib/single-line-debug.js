...
function generateSLDComponents() {
    ...
    //TODO добавить переключатель для выбора точек останова - M0 M1 или все MSG`
    //сделать button вместо checkbox и сделать для него css c тремя состояниями
    ...
    function changePauseLevel() {
        ...
        if (View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.level > 2) {
            View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.level = 0;
        }
        View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.innerText = View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.buttonText[View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.level];
        changePauseBackground();
    ...
    function changePauseBackground() {
        var jo = (View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.level + 1)* 33 - 5;
    ...
    View.sinumerikView.singleLineDebugHelpDiv.G42div.lineDiv.style.background = 'darkseagreen';
    ...
    View.sinumerikView.singleLineDebugHelpDiv.C_As_Rot.style.float = 'left'
    View.sinumerikView.singleLineDebugHelpDiv.C_As_Rot.className = 'sinumerikBorderDiv'
    ...
    View.sinumerikView.singleLineDebugLeftFootDiv.appendChild(View.sinumerikView.singleLineDebugHelpDiv.C_As_Rot)
...
function displayCanvasElements(ctx) {
    ...
    if (!View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.checked && View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.level < 2) {
        ...
        // console.log(array_of_little_elements);
        var idx = 0;
        //TODO Сделать какой-то регулятор скорости отрисовки.
        var timeout = 5;
        var pause = 0;
        var timeout_K = 1;
        if (!View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.checked) {
        ...
        ctx_timeout = setTimeout(function draw() {
            // console.log(array_of_little_elements[idx]);
            if (pause != 1) {
                pause = ctx_draw(array_of_little_elements[idx]);

                timeout = timeout_K * (array_of_little_elements[idx].type == 'G0' ? 5 : 50);
                ...
                if (timeout > 0 && array_of_little_elements[idx].type == 'G1') {
                    ...
                    if (length_of_little_element < 0.3) {
                        ...
                        timeout /= 2 * length_of_little_element
                    }
                }
                //endregion
            ...
            if (idx < array_of_little_elements.length) {
    ...
    function ctx_draw(element) {
        if (View.sinumerikView.singleLineDebugInfoDiv.slowDebugCheck.checked || View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.level == 2) {
            var pause_level = 3;
            if (element.type == 'msg') {
                //TODO Добавить обработчик на button
                msg_div(element);
                pause_level = 2;
            }

            if (element.type == 'pause') {
                pause_level = element.value == 'M0' ? 0 : 1;
            }

            if (pause_level <= View.sinumerikView.singleLineDebugInfoDiv.pauseCheck.level) {
                var msg = View.sinumerikView.msgDiv ? View.sinumerikView.msgDiv.msg.innerText : "";
                var msg_type = element.type == 'pause' ? element.value : 'PAUSE';
                msg_div({value: msg + ' _ _ ' + msg_type});
                return 1;
            }
        }
        // console.log(element)
