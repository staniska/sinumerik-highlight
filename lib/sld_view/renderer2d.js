...
import { canvasElementColor, drawAxes, transformByRange, sldScrollToElement } from '../single-line-debug'
...
function _buildGeometry(elementLimit = Infinity, sourceElements = null) {
    ...
    let elementCount = 0
    canvas.forEach(el => {
        // elementLimit lets slow debug reveal elements one-by-one.
        // Count ALL element types (including msg/pause) to match drawnElementsNums.
        if (elementCount >= elementLimit) return
        elementCount++
