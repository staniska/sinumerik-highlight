...
export function init2DRenderer(container) {
    ...
    blankMaterial   = new THREE.MeshBasicMaterial({ color: BLANK_COLOR,   side: THREE.DoubleSide })
    contourMaterial = new THREE.MeshBasicMaterial({ color: CONTOUR_COLOR, side: THREE.DoubleSide })
...
export function destroy2DRenderer() {
    ...
    mainMaterial = hoverMaterial = blankMaterial = contourMaterial = null
    segmentElementIds = []
...
function _onMouseUp(event) {
    ...
    if (!wasDragging && currentHoveredId !== null) {
        _handleClick()
    }
}
...
function _hitTest(offsetX, offsetY) {
    ...
    const w = glRenderer.domElement.width
    const h = glRenderer.domElement.height
...
function _handleClick() {
    ...
    if (!el.mainRow) {
        // element from main file — scroll editor to source line
        const Editor = atom.workspace.getActiveTextEditor()
        if (Editor) Editor.setCursorBufferPosition([el.row, 0])
    } else {
        // element from subroutine — Stage 5: show popup
