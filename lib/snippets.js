...
export async function loadSnippet(filepath) {
    const snippets = []
    const files = await fsPromises.readdir(filepath)
    for (const file of files) {
        if (path.extname(file) === '.json') {
            try {
                const text = await fsPromises.readFile(filepath + '/' + file, 'utf8')
...
export function getSnippets () {
    ...
    if (!View.sinumerikView.programmData[fileName] ||
        View.sinumerikView.programmData[fileName].machine === undefined) { return }
    ...
    const closeSnippetsMenu = () => {
        if (!sinumerikSnippetDiv.length) { return }
    ...
    if (sinumerikSnippetDiv.length) {
        ...
        View.sinumerikView.snippetsDiv.inputIcon.className = 'icon icon-search sinumerikSnippetsInputIcon'
        ...
        View.sinumerikView.snippetsDiv.closeButton = document.createElement('button')
        View.sinumerikView.snippetsDiv.closeButton.className = 'sinumerikSnippetsCloseButton'
        View.sinumerikView.snippetsDiv.closeButton.innerText = '×'
        View.sinumerikView.snippetsDiv.closeButton.title = 'Close (Esc)'
        View.sinumerikView.snippetsDiv.closeButton.addEventListener('click', () => {
            closeSnippetsMenu()
        })
        View.sinumerikView.snippetsDiv.appendChild(View.sinumerikView.snippetsDiv.closeButton)

        View.sinumerikView.snippetsDiv.input.addEventListener('keydown', (key) => {
            if (key.key === 'Escape') {
                key.preventDefault()
                key.stopPropagation()
                closeSnippetsMenu()
        ...
        View.sinumerikView.snippetsDiv.input.addEventListener('keyup', (key) => {
            ...
            if (key.key === 'Escape') {
                return
            } else if (key.key === 'ArrowRight' || key.key === 'Enter') {
        ...
        View.sinumerikView.snippetsDiv.snippetsOl.className = 'mark-active sinumerikSnippetsOl'
        ...
        const editorHeight = atom.workspace.getActiveTextEditor().getHeight()
        const clientRect = View.sinumerikView.snippetsDiv.getBoundingClientRect()
        if (clientRect.top + clientRect.height > editorHeight) {
            View.sinumerikView.snippetsDiv.style.top = `${editorHeight - clientRect.height}px`
        }
    }
    ...
    function getSnipList() {
        ...
        if (!snipArray.length) {
            ...
            emptyLi.className = 'snippetsLiEmpty'
            emptyLi.innerText = 'No snippets for this machine'
            View.sinumerikView.snippetsDiv.snippetsOl.appendChild(emptyLi)
            return
        }
    ...
    function getSnipArray() {
        ...
        const machineSnippets = View.sinumerikView.machineData.snippets[machineName]
        if (!machineSnippets) { return snipArray }

        machineSnippets.forEach((snippetsPath) => {
...
function snippetForm(snip, cursorScreenPosition, cursorPosition) {
    ...
    if (snip.variables && snip.variables.length) {
        snip.variables.forEach((snipVar, idx) => {
            ...
            snipForm[snipVar.name].name.className = 'sinumerikSnippetVarName'
            ...
            snipForm[snipVar.name].input.className = 'sinumerikSnippetVarInput'
    ...
    const editorHeight = atom.workspace.getActiveTextEditor().getHeight()
    if (snipForm.clientHeight + snipForm.getBoundingClientRect().top > editorHeight) {
        snipForm.style.top = `${editorHeight - snipForm.clientHeight}px`
    }
