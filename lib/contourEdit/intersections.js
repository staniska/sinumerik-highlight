...
import {createRoundedPointsObj} from "./tools/canvasArea";
        ...
        .forEach((el, idx) => {
            contour
                ...
                .forEach(el2 => {
                    ...
                    checkStartEndPointsEq(el, el2, intersections)
...
const checkStartEndPointsEq = (el1, el2, intersections) => {
    const {x, y} = getFrame()
    const pairs = [
        [el1.start[x], el1.start[y], el2.start[x], el2.start[y]],
        [el1.end[x], el1.end[y], el2.start[x], el2.start[y]],
        [el1.start[x], el1.start[y], el2.end[x], el2.end[y]],
        [el1.end[x], el1.end[y], el2.end[x], el2.end[y]],
    ]
    pairs.forEach((p, idx) => {
        if (p[0] === p[2] && p[1] === p[3]) {
            intersections.push({xIntersection: p[0], yIntersection: p[1], el1, el2})
        }
    })
}
...
const findLinesIntersection = (el1, el2) => {
    ...
    if (eq1.b === 0 || eq2.b === 0) {
        if (eq1.b === 0 && eq2.b === 0) {
            if (Math.abs(el1.start[x] - el2.start[x]) < ACCURACY) {
                const intersections = []
...
const findArcsIntersection = (el1, el2) => {
    ...
    if (distance < ACCURACY) return intersections
    if (distance > el1.radius + el2.radius + ACCURACY) return intersections
    if (distance < Math.abs(el1.radius - el2.radius) - ACCURACY) return intersections

    const a = (el1.radius ** 2 - el2.radius ** 2 + distance ** 2) / (2 * distance)
    const h = Math.sqrt(Math.max(0, el1.radius ** 2 - a ** 2))
...
export const checkPointBelongsArc = (arc, p) => {
    if (!arc.radius || !arc.center) updateArcProperties(arc)
...
export const getEquationOfLine = (el) => {
    ...
    if (Math.abs(el.start[x] - el.end[x]) < ACCURACY) {
        return {a: 1, b: 0, c: -el.start[x]}
    }

    if (Math.abs(el.start[y] - el.end[y]) < ACCURACY) {
