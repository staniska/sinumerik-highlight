...
import {highlightBoundingRow, clearBoundingHighlight} from "../boundingHighlight";
...
export const updateContourEditData = async () => {
    const fileName = atom.workspace.getActiveTextEditor().getPath()

    if (View.sinumerikView.contourEditData) {
        View.sinumerikView.contourEditData.contours = await load_contours()
    }
    ...
    const contourElementsMeta = View.sinumerikView.parseData?.contourElementsMeta ?? {}
    Object.keys(View.sinumerikView.parseData?.contourElements ?? {}).filter(k => {
        if (k.startsWith('BOUNDING_')) return true
        const basename = contourElementsMeta[k]?.basename
        if (!basename) return false
        return (blankName && basename === blankName)
            || (contourName && basename === contourName)
    }).forEach(k => {
        const isBounding = k.startsWith('BOUNDING_')
        const meta = View.sinumerikView.boundingContourMeta?.[k]
        const label = isBounding && meta
            ? `${meta.subName} · ${meta.callRow + 1}`
            : k
        const classes = ['editContour_foldersLi']
        if (isBounding) classes.push('editContour_foldersLi--bounding')
        ...
        View.sinumerikView.contourEditRightContainer.contoursContainer.getContours[k].addEventListener('click', (e) => {
            ...
            if (View.sinumerikView.contourEditData.editContour.filter(el => el?.sourceFile === k).length) {
                ...
                if (isBounding) clearBoundingHighlight()
            } else {
                ...
                if (isBounding && meta?.callerFile != null && meta?.callRow != null) {
                    highlightBoundingRow(meta.callerFile, meta.callRow)
                }
            }
            updateIntersections()
            handleNormalize()
    ...
    View.sinumerikView.contourEditData.contours.forEach(contours => {
        const folderLabel = contours.filename.replace(/^.*[\\/]/, '').replace(/\.[^.]+$/, '')
        const folderLi = create_element(['editContour_foldersLi'], contoursOl, 'li', folderLabel)
        const foldersLiIcon = create_element(['icon-plus', 'editContour_foldersLiIcon'], folderLi)
        ...
        foldersLiIcon.classList.toggle('icon-plus', folderOl.style.display === 'none')
        foldersLiIcon.classList.toggle('icon-dash', folderOl.style.display !== 'none')
        ...
        contours.contours.forEach(contour => {
            const el = create_element(['editContour_contoursLi'], folderOl, 'li', contour.name)
...
export const save_contours = () => {
    let fileName = atom.workspace.getActiveTextEditor().getPath()
...
const save = async () => {
    ...
    try {
        await fsPromises.writeFile(contoursPath, JSON.stringify(formattedContours))
...
export const createFileFromArea = async () => {
    ...
    if (!canceled && filePath) {
        ...
        contour.forEach((el, idx) => {
            if (el.type === 'line') {
                const point = getDistance(el.end, lastPoint) < POINT_EPSILON ? el.start : el.end
                fileText.push(`G1 ${x}${coordMult(point, x)} ${y}${coordMult(point, y)}`)
                lastPoint = point
            } else {
                const point = getDistance(el.end, lastPoint) < POINT_EPSILON ? el.start : el.end
                const reverse = getDistance(el.end, lastPoint) < POINT_EPSILON ? 1 : 0
