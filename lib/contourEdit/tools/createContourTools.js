...
export const contourTools = () => {
    ...
    precision.x1000 = create_element(['contourEditRadio'], precision, 'input', '1');
    precision.x500 = create_element(['contourEditRadio'], precision, 'input', '0.5');
    precision.x100 = create_element(['contourEditRadio'], precision, 'input', '0.1');
    precision.x10 = create_element(['contourEditRadio'], precision, 'input', '0.01');

    precision.querySelectorAll('input').forEach((el, idx) => {
        el.id = el.textContent
        el.name = 'precision'
        el.label = create_element(['contourEditRadioLabel'], null, 'label', el.textContent)
        el.after(el.label)
        el.label.setAttribute('for', el.id)
        el.type = 'radio'
        if (idx === 0) {
            el.checked = true
            View.sinumerikView.contourEditData.precision = parseFloat(el.textContent)
        }
