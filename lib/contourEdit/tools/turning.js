...
const generateProgramText = (direction, ax, cRange, contour, x, y) => {
    ...
    if (PArr.length > 2) {
        for (let i = 0; i < PArr.length - 2; i++) {
            ...
            programText.push(`  G0 ${ax2}=R${startVarNum} ` + getEquationByAx(
                ...
                ax, ax2, PArr[i + 1], startVarNum, reverse) + `+R${safetyVarNum}${diamonAx === ax ? '*2': ''}`)
    ...
    programText.push(`G0 ${ax2}=R${startVarNum} ` + getEquationByAx(
        ...
        ax, ax2, PArr[PArr.length - 1], startVarNum, reverse) + `+R${safetyVarNum}${diamonAx === ax ? '*2': ''}`)
...
const filterProgramText = (programText) => {
    ...
    //replase -- => +
    programText = programText.map(line => (line[0] === ';' ? line : line.replace(/--/g, '+')))
