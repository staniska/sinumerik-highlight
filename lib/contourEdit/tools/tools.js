...
import {createFileFromArea} from "./fs";
...
export const generateTools = () => {
    if (!View.sinumerikView.contourEditRightContainer.tools) {
        ...
        View.sinumerikView.contourEditRightContainer.contoursContainer.getContours = create_element(['editContour_contoursOl'], View.sinumerikView.contourEditRightContainer.contoursContainer, 'ol', 'Get elements from:')
        ...
        View.sinumerikView.contourEditRightContainer.processing.header = create_element(['contourEditContainerHeader'], View.sinumerikView.contourEditRightContainer.processing, 'div', 'Processing')

        View.sinumerikView.contourEditRightContainer.processing.selectArea = create_element(['contourEditButton', 'd_none'], View.sinumerikView.contourEditRightContainer.processing, 'button', 'select area')
        ...
        const selectArea = () => {
            ...
            if (doubleId !== null) {
                const contourTools = View.sinumerikView.contourEditRightContainer.tools.contourTools
                contourTools.children[0].click()
                const editContainer = contourTools.querySelector('.contourEditContainer_edit')
                if (editContainer && editContainer.classList.contains('d_none')) {
                    contourTools.querySelector('.contourEditContainer_contoursHeader').click()
                }
                const idx = View.sinumerikView.contourEditData.editContour.findIndex(el => el.id === doubleId)
                if (idx >= 0) {
                    const elementLi = document.querySelector('.editContour_elementsOl').children[idx]
                    if (elementLi) {
                        elementLi.scrollIntoView({block: 'nearest'})
                        elementLi.click()
                    }
                }
                return
        ...
        View.sinumerikView.contourEditRightContainer.processing.ol.turning.btn = create_element(['contourEditButton'], View.sinumerikView.contourEditRightContainer.processing.ol.turning, 'button', 'turning')
        ...
        View.sinumerikView.contourEditRightContainer.processing.ol.fileFromArea = create_element(['processing_Li'], View.sinumerikView.contourEditRightContainer.processing.ol, 'li')
        View.sinumerikView.contourEditRightContainer.processing.ol.fileFromArea.btn = create_element(['contourEditButton'], View.sinumerikView.contourEditRightContainer.processing.ol.fileFromArea, 'button', 'Area => file')
        View.sinumerikView.contourEditRightContainer.processing.ol.fileFromArea.btn.disabled = true
        View.sinumerikView.contourEditRightContainer.processing.ol.fileFromArea.btn.addEventListener('click', createFileFromArea)
