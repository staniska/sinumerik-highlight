...
import {getFrame, draw, DEBUG_DRAW_POINTS} from "../canvas";
...
export const checkCanvasArea = () => {
    ...
    if (DEBUG_DRAW_POINTS) animateBurnedPoints()
}
...
const burnForest = (cursorPosition, pointsObj) => {
    ...
    View.sinumerikView.contourEditData.doubleTraversalElementId = null
    const contour = View.sinumerikView.contourEditData.editContour
    ...
    const visitedElementIds = new Set([firstElPoint.parentId])
    let startElementReentered = false
    ...
    while (searching && max) {
        ...
        const nextId = nextP[0].parentId
        const lastId = burnedPoints[burnedPoints.length - 1].parentId
        if (nextId !== lastId) {
            if (visitedElementIds.has(nextId)) {
                if (nextId === firstElPoint.parentId && !startElementReentered) {
                    startElementReentered = true
                } else {
                    let intruderCount = 0
                    for (let k = burnedPoints.length - 1; k >= 0 && burnedPoints[k].parentId === lastId; k--) {
                        intruderCount++
                    }
                    if (intruderCount === 1) {
                        visitedElementIds.delete(lastId)
                    } else {
                        View.sinumerikView.contourEditData.doubleTraversalElementId = nextId
                        burnedPoints.length = 0
                        burnedElementsParts.length = 0
                        return
                    }
    ...
    const burnedPointCounts = new Map()
    burnedPoints.forEach(p => burnedPointCounts.set(p.parentId, (burnedPointCounts.get(p.parentId) || 0) + 1))
    const isSubstantial = id => (burnedPointCounts.get(id) || 0) > 1
    if (burnedIntersectionPoints.length) {
        const validIntersectionPoints = burnedIntersectionPoints
            .filter(p => p.parentElementsIds.every(isSubstantial))
        const burnedElementsPartsIds = Array.from(
            new Set(validIntersectionPoints.map(p => p.parentElementsIds).flat())
        )
        burnedElementsPartsIds.forEach(id => {
            const el = View.sinumerikView.contourEditData.editContour.filter(el => el.id === id)[0]
            const points = Array.from(new Set(validIntersectionPoints
                    .filter(p => {
