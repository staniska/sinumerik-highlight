...
const burnForest = (cursorPosition, pointsObj) => {
    ...
    // Use only elements actually in the burned path — detachedElements (tails) were
    // already stripped by the burn loop, so they won't skew the crossing parity.
    const burnedIds = new Set(burnedPoints.map(p => p.parentId))
    if (countRayCrossings(cursorPosition, contour.filter(el => burnedIds.has(el.id)), x, y) % 2 === 0) {
