...
const DEG_QUARTER = 90
const DEG_HALF = 180
const ANG_EPSILON = 1e-4
const ZOOM_SPEED = 0.001

export const DEBUG_DRAW_POINTS = false
...
export const mouseDownHandler = (e) => {
    // Drag is a left-click gesture. Right/middle-click would otherwise
    // arm a drag that the context menu then swallows the mouseup for,
    // leaving the canvas in a dragging state after the menu closes.
    ...
    if (View.sinumerikView.contourEditData.points.inFocus.length) {
        ...
        View.sinumerikView.contourEditData.points.dragging.forEach(p => {
            p.originalCoords = {...p.coords}
        })
...
const dragPointHandler = () => {
    View.sinumerikView.contourEditData.points.dragging.forEach((point, idx, dragPoints) => {
        ...
        if (lineDragPoints.filter(point => point.subType === 'end' && elements[point.idx].angBlocked).length) {
            ...
            if (Math.abs(el.ang) % DEG_QUARTER < ANG_EPSILON) {
                if (Math.abs(el.ang) % DEG_HALF < ANG_EPSILON) {
                    coords[y] = el.start[y]
                } else {
                    coords[x] = el.start[x]
                }
            } else {
                const distanceByX = Math.abs(coords[x] - el.start[x]) / Math.cos(el.ang * Math.PI / DEG_HALF)
                const distanceByY = Math.abs(coords[y] - el.start[y]) / Math.sin(el.ang * Math.PI / DEG_HALF)
                if (distanceByX > distanceByY) {
                    const newYValue = parseFloat(el.start[y]) + (coords[x] - el.start[x]) * Math.tan(el.ang * Math.PI / DEG_HALF)
                    if (!isFinite(newYValue)) return
                    coords[y] = newYValue
                } else {
                    const newXValue = parseFloat(el.start[x]) + (coords[y] - el.start[y]) / Math.tan(el.ang * Math.PI / DEG_HALF)
        ...
        const checkPoints = point.subType === 'start' ? ['end']
            : point.subType === 'end'    ? ['start']
            : ['start', 'end']
        const wouldMerge = checkPoints.some(sub => {
            const other = el[sub]
            return Math.sqrt((coords[x] - other[x]) ** 2 + (coords[y] - other[y]) ** 2) < 1
        })
        if (wouldMerge) {
            // Merging start with end of an arc produces a degenerate full
            // circle that breaks updateArcProperties / checkPointBelongsArc.
            // Snap the dragged arc point back to where the drag started so
            // the user clearly sees the move was rejected.
            if (el.type === 'arc' && point.originalCoords) {
                point.coords = {...point.originalCoords}
        ...
        if (!elements[point.idx].angBlocked || point.subType === 'start') {
            ...
            const el = elements[point.idx]
            if (Math.abs(Math.abs(el.ang - Math.atan2((el.end[y] - el.start[y]), (el.end[x] - el.start[x])) / Math.PI * DEG_HALF) - DEG_HALF) < ANG_EPSILON) {
                setAng(View.sinumerikView.contourEditData.editContour[point.idx])
            }
        }
...
const mouseUpHandler = () => {
    ...
    if (points.inFocus.length) {
        ...
        // inFocus may contain other points of the dragged element itself
        // (e.g. the opposite endpoint of the same arc) — snapping the
        // dragged point onto them would merge start with end and break
        // the geometry. Exclude every point belonging to a dragged
        // element so snap only connects to truly different contour items.
        const draggedIdxs = new Set(points.dragging.map(d => d.idx))
        const safeSnap = points.inFocus.find(f => !draggedIdxs.has(f.idx))
        if (safeSnap) {
...
export const scrollCanvas = (event, deltaY) => {
    ...
    if (delta) {
        ...
        View.sinumerikView.contourEditData.canvasFrame.scale += delta * -ZOOM_SPEED * View.sinumerikView.contourEditData.canvasFrame.scale
...
export const setAng = (el) => {
    if (el.type === 'line') {
        const {x, y} = getFrame()
        el.ang = (Math.atan2((el.end[y] - el.start[y]), (el.end[x] - el.start[x])) / Math.PI * DEG_HALF).toFixed(4)
    }
}

export const resizeContourEditCanvas = () => {
    ...
    if (oldWidth > 0 && oldHeight > 0) {
        View.sinumerikView.contourEditData.canvasFrame.trans[0] *= canvas.width / oldWidth
        View.sinumerikView.contourEditData.canvasFrame.trans[1] *= canvas.height / oldHeight
        draw()
    }
}
...
export const draw = (event) => {
    ...
    const canvasFrame = View.sinumerikView.contourEditData.canvasFrame
    let savedFrame = View.sinumerikView.contourEditData.savedFrames.find(el => el.filename === filename)
    if (!savedFrame) {
        savedFrame = {filename, trans: [canvasFrame.trans[0], canvasFrame.trans[1]], scale: canvasFrame.scale}
        View.sinumerikView.contourEditData.savedFrames.push(savedFrame)
    }
    savedFrame.trans[0] = canvasFrame.trans[0]
    savedFrame.trans[1] = canvasFrame.trans[1]
    savedFrame.scale = canvasFrame.scale
    ...
    if (View.sinumerikView.contourEditData.editContour.length) {
        ...
        const propEditNode = View.sinumerikView.contourEditFootContainer
            .querySelector('.contourEditElementProperties_id')
        const propEditId = propEditNode ? propEditNode.innerText : null
        const nearCursorIdx = View.sinumerikView.contourEditData.elements.nearCursor
        const doubleId = View.sinumerikView.contourEditData.doubleTraversalElementId

        const isHighlighted = (idx, el) => (
            elements[idx].classList.contains('editContourElement_highlight') ||
            idx === nearCursorIdx ||
            (propEditId !== null && propEditId === '' + (idx + 1)) ||
            el.id === doubleId
        )

        const drawOne = (el, idx) => {
            let weight = 'normal'
            let colorType = 'temp'
            if (elements[idx].classList.contains('editContourElement_highlight')) {
                weight = 'bold'
            }
            if (idx === nearCursorIdx) {
                weight = 'bold'
                colorType = 'highlight'
            }
            if (propEditId !== null && propEditId === '' + (idx + 1)) {
                weight = 'bold'
            }
            if (el.id === doubleId) {
                weight = 'bold'
                colorType = 'doubleTraversal'
            }
        ...
        // Three passes so the cursor-hovered element always lands on top:
        //   1) plain elements,
        //   2) other highlights (selected from the list / elementProperties /
        //      doubleTraversal),
        //   3) the nearCursor pick — wins z-order over every overlap.
        View.sinumerikView.contourEditData.editContour.forEach((el, idx) => {
            if (!isHighlighted(idx, el)) drawOne(el, idx)
        })
        View.sinumerikView.contourEditData.editContour.forEach((el, idx) => {
            if (isHighlighted(idx, el) && idx !== nearCursorIdx) drawOne(el, idx)
        })
        if (nearCursorIdx >= 0 && View.sinumerikView.contourEditData.editContour[nearCursorIdx]) {
            drawOne(View.sinumerikView.contourEditData.editContour[nearCursorIdx], nearCursorIdx)
        }
    }
    ...
    if (View.sinumerikView.contourEditData.points.intersectionInFocus) {
        drawElement(ctx, View.sinumerikView.contourEditData.points.intersectionInFocus, color('intersection'), 'bold')
    }

    //
    if (DEBUG_DRAW_POINTS) {
        if (View.sinumerikView.contourEditData.points.roundedObj) {
            const elementColor = (id) => `hsl(${(id * 137) % 360}, 65%, 50%)`
            const weights = ['bold', 'medium', null]
            Object.values(View.sinumerikView.contourEditData.points.roundedObj).forEach(col => {
                Object.values(col).forEach(pts => {
                    const active = pts.filter(p => !p.isTail)
                    active.forEach((p, i) => {
                        drawElement(ctx, {...p, type: 'point'}, elementColor(p.parentId), weights[Math.min(i, 2)])
                    })
                })
            })
        }
...
const drawElementSymbol = (symbol, ctx, event) => {
    ...
    if (symbol === 'arc') {
        ctx.arc(event.offsetX + 20, event.offsetY + 8, 12, -10 / DEG_HALF * Math.PI, 130 / DEG_HALF * Math.PI)
    }
...
const color = (type) => {
    ...
    if (type === 'doubleTraversal') return 'red'
...
export const isNumber = (...args) => args.every(value => Number.isFinite(value))
...
export const setCursorPosition = (event) => {
    ...
    const round = (number) => {
        ...
        return (Math.round(number / precision) * precision).toFixed(Math.max(0, Math.ceil(-Math.log10(precision))))
    }
...
const drawCursorPosition = (ctx, event) => {
    ...
    ctx.fillStyle = getComputedStyle(ctx.canvas).getPropertyValue('--cursor-text-color').trim() || 'black'
    ctx.font = "300 18px Verdana"

    ctx.fillText(`${x}: ${(x === diamonAx) ? 'Ø' : ''} ${cursorPosition[x] * ((x === diamonAx) ? 2 : 1)}`, event.offsetX + 10, event.offsetY - 30)
    ctx.fillText(`${y}: ${(y === diamonAx) ? 'Ø' : ''} ${cursorPosition[y] * ((y === diamonAx) ? 2 : 1)}`, event.offsetX + 10, event.offsetY - 10)
...
const drawCursorConfines = (event, ctx) => {
    ...
    ctx.fillText(confinesText.toUpperCase(), event.offsetX + 10, event.offsetY + 30)
}
