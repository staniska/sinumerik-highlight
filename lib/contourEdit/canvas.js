...
export const draw = (event) => {
    ...
    // Отрисовка точек на элементах
    // if (View.sinumerikView.contourEditData.points.rounded.length) {
    //     View.sinumerikView.contourEditData.points.rounded.forEach(point => {
    //         drawElement(ctx, {...point, type: 'point'}, 'green', 'normal')
    //     })
    // }

    if (View.sinumerikView.contourEditData.points.burned.length) {
        View.sinumerikView.contourEditData.visibleActions = true
        View.sinumerikView.contourEditData.points.burned.forEach((point) => {
            // if (idx % 10 !==0 ) return
            drawElement(ctx, {...point, type: 'point'}, 'orange', 'bold')
        })
    }
...
const drawArc = (ctx, arc, element) => {
    ...
    ctxArc(ctx, element.center, element.radius, element.startAng, element.endAng, element.ccw)
...
// const checkPointInElements = (point, elements) => {
//     if (elements === undefined) {
//         try {
//             elements = View.sinumerikView.contourEditData.editContour.filter(el => point.parentElementsIds.includes(el.id))
//         } catch (e) {
//             elements = []
//         }
//     }
...
//         }).flat()
//
//     return (elementsPoints.filter(p => JSON.stringify(p) === JSON.stringify(point.coords)).length > 0)
// }
