...
const selectIntersectionPointsAtElement = (id) => {
    ...
    const elementsPoints = View.sinumerikView.contourEditData.editContour
        .map(el => [el.start, el.end].map(p => [p[x].toFixed(3), p[y].toFixed(3)]))
        .flat()
        .map(p => JSON.stringify(p))
    View.sinumerikView.contourEditData.points.intersectionsAtElement = intersectionPoints
        .filter(p => elementsPoints.indexOf(JSON.stringify([p.coords[x].toFixed(3), p.coords[y].toFixed(3)])) < 0)
