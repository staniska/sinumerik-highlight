...
export const elementProperties = (el, idx) => {
    ...
    if (!container.elementProperties) {
        ...
        container.elementProperties.header.text = create_element(['contourEditElementProperties_header_text'], container.elementProperties.header, 'div', 'ElementProperties')
    ...
    if (!el) {
        View.sinumerikView.contourEditData.points.intersectionsAtElement = []
        View.sinumerikView.contourEditData.elements.propertyEdit = -1
        container.removeChild(container.elementProperties)
        delete container.elementProperties
        return
    }

    View.sinumerikView.contourEditData.elements.propertyEdit = idx
    ...
    selectIntersectionPointsAtElement(idx)
}

const selectIntersectionPointsAtElement = (id) => {
    const {x, y} = getFrame()
    const intersectionPoints = View.sinumerikView.contourEditData.points.intersection
    const selectedEl = View.sinumerikView.contourEditData.editContour[id]
    // Only exclude points that coincide with the endpoints of the selected
    // element — dividing there would produce a zero-length piece. Endpoints
    // of other elements that happen to lie mid-way through the selected
    // element are valid split points.
    const selectedEndpoints = selectedEl
        ? [selectedEl.start, selectedEl.end]
            .map(p => JSON.stringify([p[x].toFixed(3), p[y].toFixed(3)]))
        : []
    View.sinumerikView.contourEditData.points.intersectionsAtElement = intersectionPoints
        .filter(p => selectedEndpoints.indexOf(JSON.stringify([p.coords[x].toFixed(3), p.coords[y].toFixed(3)])) < 0)
        .filter(p => id !== undefined ? (p.parentElementsIds[0] === id || p.parentElementsIds[1] === id) : true)
}


const radius = (el, container, idx) => {
    const radiusDiv = create_element(['contourEditElementProperties_angle'], container)
    radiusDiv.header = create_element(['contourEditElementProperties_angleHeader'], radiusDiv, 'div', 'Radius:')
...
const ang = (el, container, idx) => {
    ...
    angDiv.header = create_element(['contourEditElementProperties_angleHeader'], angDiv, 'div', 'Ang:')
...
const point = (el, pointType, container, idx) => {
    ...
    pointDiv.header = create_element(['contourEditElementProperties_pointHeader'], pointDiv, 'div', pointType)
    ...
    Object.keys(el[pointType]).sort().forEach(coord => {
        ...
        coordDiv.axis = create_element(['contourEditElementProperties_coordName'], coordDiv, 'div', `${diamon_K === 2 ? 'Ø' : ''}${coord}`)
        ...
        coordDiv.value.addEventListener('change', () => {
            ...
            // For arcs, reject coords that would merge start with end —
            // a full-circle arc breaks updateArcProperties / hit-testing.
            if (el.type === 'arc') {
                const {x, y} = getFrame()
                const opposite = pointType === 'start' ? el.end : el.start
                const dist = Math.sqrt(
                    (newCoords[x] - opposite[x]) ** 2 +
                    (newCoords[y] - opposite[y]) ** 2
                )
                if (dist < 1) {
                    coordDiv.value.value = el[pointType][coord] * diamon_K
                    alertDialog('Arc start and end cannot coincide')
                    return
                }
            }
