...
import {elementProperties, changeRadius} from "./footer/elementProperties";
...
const contextMenuHandle = () => {
    if (View.sinumerikView.contourEditData.elements.propertyEdit >= 0) {
        const intersection = View.sinumerikView.contourEditData.points.intersectionInFocus
        if (intersection) {
            const menu = contextMenu([
                { label: 'Divide', click: () => divideHandle(intersection) },
            ])
            menu.popup()
        }
    }
...
const divideHandle = (intersection) => {
    const idx = View.sinumerikView.contourEditData.elements.propertyEdit
    elementProperties()
    const el1 = {
        ...View.sinumerikView.contourEditData.editContour[idx],
        id: undefined,
        end: intersection.coords
    }
    const el2 = {
        ...View.sinumerikView.contourEditData.editContour[idx],
        id: undefined,
        start: intersection.coords
    }
    if (el1.type === 'arc') {
        changeRadius(el1, el1.radius, true)
        changeRadius(el2, el2.radius, true)
    }
...
const checkCanvasElement = () => {
    ...
    // Overlapping-lines tiebreaker: if the 1 mm cell of the chosen point
    // hosts more than one element and all candidates are lines, route the
    // pick by the sign of (end-start) × (cursor-start) relative to the
    // lowest-id line. Positive → first line, negative → next one. Gives a
    // stable choice based on which side of the line the cursor is on.
    const cell = roundedPoints[nearestPoint.coords[x]]?.[nearestPoint.coords[y]] || []
    const uniqueIds = [...new Set(cell.map(p => p.parentId))].sort((a, b) => a - b)
    if (uniqueIds.length >= 2) {
        const editContour = View.sinumerikView.contourEditData.editContour
        const firstEl  = editContour[uniqueIds[0]]
        const secondEl = editContour[uniqueIds[1]]
        if (firstEl?.type === 'line' && secondEl?.type === 'line') {
            const cursor = View.sinumerikView.contourEditData.cursorPosition
            const dx = firstEl.end[x] - firstEl.start[x]
            const dy = firstEl.end[y] - firstEl.start[y]
            const cx = cursor[x]    - firstEl.start[x]
            const cy = cursor[y]    - firstEl.start[y]
            const sign = dx * cy - dy * cx
            return sign < 0 ? uniqueIds[1] : uniqueIds[0]
        }
    }
...
export const getBlankContour = async () => {
    const Editor = atom.workspace.getActiveTextEditor()
    ...
    try {
        ...
        return getContourFromText(text.split('\n'))
